# Graph Report - palex-platform  (2026-07-09)

## Corpus Check
- 309 files · ~456,191 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6357 nodes · 11596 edges · 542 communities (527 shown, 15 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 144 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4274165a`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Community 0|Community 0]]
- [[_COMMUNITY_Community 1|Community 1]]
- [[_COMMUNITY_Community 2|Community 2]]
- [[_COMMUNITY_Community 3|Community 3]]
- [[_COMMUNITY_Community 4|Community 4]]
- [[_COMMUNITY_Community 5|Community 5]]
- [[_COMMUNITY_Community 6|Community 6]]
- [[_COMMUNITY_Community 7|Community 7]]
- [[_COMMUNITY_Community 8|Community 8]]
- [[_COMMUNITY_Community 9|Community 9]]
- [[_COMMUNITY_Community 10|Community 10]]
- [[_COMMUNITY_Community 11|Community 11]]
- [[_COMMUNITY_Community 12|Community 12]]
- [[_COMMUNITY_Community 13|Community 13]]
- [[_COMMUNITY_Community 14|Community 14]]
- [[_COMMUNITY_Community 15|Community 15]]
- [[_COMMUNITY_Community 16|Community 16]]
- [[_COMMUNITY_Community 17|Community 17]]
- [[_COMMUNITY_Community 18|Community 18]]
- [[_COMMUNITY_Community 19|Community 19]]
- [[_COMMUNITY_Community 20|Community 20]]
- [[_COMMUNITY_Community 21|Community 21]]
- [[_COMMUNITY_Community 22|Community 22]]
- [[_COMMUNITY_Community 23|Community 23]]
- [[_COMMUNITY_Community 24|Community 24]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 28|Community 28]]
- [[_COMMUNITY_Community 29|Community 29]]
- [[_COMMUNITY_Community 30|Community 30]]
- [[_COMMUNITY_Community 31|Community 31]]
- [[_COMMUNITY_Community 32|Community 32]]
- [[_COMMUNITY_Community 33|Community 33]]
- [[_COMMUNITY_Community 34|Community 34]]
- [[_COMMUNITY_Community 35|Community 35]]
- [[_COMMUNITY_Community 36|Community 36]]
- [[_COMMUNITY_Community 37|Community 37]]
- [[_COMMUNITY_Community 38|Community 38]]
- [[_COMMUNITY_Community 39|Community 39]]
- [[_COMMUNITY_Community 40|Community 40]]
- [[_COMMUNITY_Community 41|Community 41]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 45|Community 45]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 65|Community 65]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 68|Community 68]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 126|Community 126]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 129|Community 129]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 132|Community 132]]
- [[_COMMUNITY_Community 133|Community 133]]
- [[_COMMUNITY_Community 134|Community 134]]
- [[_COMMUNITY_Community 138|Community 138]]
- [[_COMMUNITY_Community 148|Community 148]]
- [[_COMMUNITY_Community 149|Community 149]]
- [[_COMMUNITY_Community 153|Community 153]]
- [[_COMMUNITY_Community 154|Community 154]]
- [[_COMMUNITY_Community 155|Community 155]]
- [[_COMMUNITY_Community 156|Community 156]]
- [[_COMMUNITY_Community 157|Community 157]]
- [[_COMMUNITY_Community 158|Community 158]]
- [[_COMMUNITY_Community 159|Community 159]]
- [[_COMMUNITY_Community 160|Community 160]]
- [[_COMMUNITY_Community 161|Community 161]]
- [[_COMMUNITY_Community 162|Community 162]]
- [[_COMMUNITY_Community 163|Community 163]]
- [[_COMMUNITY_Community 164|Community 164]]
- [[_COMMUNITY_Community 165|Community 165]]
- [[_COMMUNITY_Community 166|Community 166]]
- [[_COMMUNITY_Community 167|Community 167]]
- [[_COMMUNITY_Community 168|Community 168]]
- [[_COMMUNITY_Community 169|Community 169]]
- [[_COMMUNITY_Community 170|Community 170]]
- [[_COMMUNITY_Community 171|Community 171]]
- [[_COMMUNITY_Community 172|Community 172]]
- [[_COMMUNITY_Community 173|Community 173]]
- [[_COMMUNITY_Community 174|Community 174]]
- [[_COMMUNITY_Community 175|Community 175]]
- [[_COMMUNITY_Community 176|Community 176]]
- [[_COMMUNITY_Community 177|Community 177]]
- [[_COMMUNITY_Community 178|Community 178]]
- [[_COMMUNITY_Community 179|Community 179]]
- [[_COMMUNITY_Community 180|Community 180]]
- [[_COMMUNITY_Community 181|Community 181]]
- [[_COMMUNITY_Community 182|Community 182]]
- [[_COMMUNITY_Community 183|Community 183]]
- [[_COMMUNITY_Community 184|Community 184]]
- [[_COMMUNITY_Community 185|Community 185]]
- [[_COMMUNITY_Community 186|Community 186]]
- [[_COMMUNITY_Community 187|Community 187]]
- [[_COMMUNITY_Community 188|Community 188]]
- [[_COMMUNITY_Community 189|Community 189]]
- [[_COMMUNITY_Community 190|Community 190]]
- [[_COMMUNITY_Community 191|Community 191]]
- [[_COMMUNITY_Community 192|Community 192]]
- [[_COMMUNITY_Community 193|Community 193]]
- [[_COMMUNITY_Community 194|Community 194]]
- [[_COMMUNITY_Community 195|Community 195]]
- [[_COMMUNITY_Community 196|Community 196]]
- [[_COMMUNITY_Community 197|Community 197]]
- [[_COMMUNITY_Community 198|Community 198]]
- [[_COMMUNITY_Community 199|Community 199]]
- [[_COMMUNITY_Community 200|Community 200]]
- [[_COMMUNITY_Community 201|Community 201]]
- [[_COMMUNITY_Community 202|Community 202]]
- [[_COMMUNITY_Community 203|Community 203]]
- [[_COMMUNITY_Community 204|Community 204]]
- [[_COMMUNITY_Community 205|Community 205]]
- [[_COMMUNITY_Community 206|Community 206]]
- [[_COMMUNITY_Community 207|Community 207]]
- [[_COMMUNITY_Community 208|Community 208]]
- [[_COMMUNITY_Community 209|Community 209]]
- [[_COMMUNITY_Community 210|Community 210]]
- [[_COMMUNITY_Community 211|Community 211]]
- [[_COMMUNITY_Community 212|Community 212]]
- [[_COMMUNITY_Community 213|Community 213]]
- [[_COMMUNITY_Community 214|Community 214]]
- [[_COMMUNITY_Community 215|Community 215]]
- [[_COMMUNITY_Community 216|Community 216]]
- [[_COMMUNITY_Community 217|Community 217]]
- [[_COMMUNITY_Community 218|Community 218]]
- [[_COMMUNITY_Community 219|Community 219]]
- [[_COMMUNITY_Community 220|Community 220]]
- [[_COMMUNITY_Community 221|Community 221]]
- [[_COMMUNITY_Community 222|Community 222]]
- [[_COMMUNITY_Community 223|Community 223]]
- [[_COMMUNITY_Community 224|Community 224]]
- [[_COMMUNITY_Community 225|Community 225]]
- [[_COMMUNITY_Community 226|Community 226]]
- [[_COMMUNITY_Community 227|Community 227]]
- [[_COMMUNITY_Community 228|Community 228]]
- [[_COMMUNITY_Community 229|Community 229]]
- [[_COMMUNITY_Community 230|Community 230]]
- [[_COMMUNITY_Community 231|Community 231]]
- [[_COMMUNITY_Community 232|Community 232]]
- [[_COMMUNITY_Community 233|Community 233]]
- [[_COMMUNITY_Community 234|Community 234]]
- [[_COMMUNITY_Community 235|Community 235]]
- [[_COMMUNITY_Community 236|Community 236]]
- [[_COMMUNITY_Community 237|Community 237]]
- [[_COMMUNITY_Community 238|Community 238]]
- [[_COMMUNITY_Community 239|Community 239]]
- [[_COMMUNITY_Community 240|Community 240]]
- [[_COMMUNITY_Community 241|Community 241]]
- [[_COMMUNITY_Community 242|Community 242]]
- [[_COMMUNITY_Community 243|Community 243]]
- [[_COMMUNITY_Community 244|Community 244]]
- [[_COMMUNITY_Community 245|Community 245]]
- [[_COMMUNITY_Community 246|Community 246]]
- [[_COMMUNITY_Community 247|Community 247]]
- [[_COMMUNITY_Community 248|Community 248]]
- [[_COMMUNITY_Community 249|Community 249]]
- [[_COMMUNITY_Community 250|Community 250]]
- [[_COMMUNITY_Community 251|Community 251]]
- [[_COMMUNITY_Community 252|Community 252]]
- [[_COMMUNITY_Community 253|Community 253]]
- [[_COMMUNITY_Community 254|Community 254]]
- [[_COMMUNITY_Community 255|Community 255]]
- [[_COMMUNITY_Community 256|Community 256]]
- [[_COMMUNITY_Community 257|Community 257]]
- [[_COMMUNITY_Community 258|Community 258]]
- [[_COMMUNITY_Community 259|Community 259]]
- [[_COMMUNITY_Community 260|Community 260]]
- [[_COMMUNITY_Community 261|Community 261]]
- [[_COMMUNITY_Community 262|Community 262]]
- [[_COMMUNITY_Community 263|Community 263]]
- [[_COMMUNITY_Community 264|Community 264]]
- [[_COMMUNITY_Community 265|Community 265]]
- [[_COMMUNITY_Community 266|Community 266]]
- [[_COMMUNITY_Community 267|Community 267]]
- [[_COMMUNITY_Community 268|Community 268]]
- [[_COMMUNITY_Community 269|Community 269]]
- [[_COMMUNITY_Community 270|Community 270]]
- [[_COMMUNITY_Community 271|Community 271]]
- [[_COMMUNITY_Community 272|Community 272]]
- [[_COMMUNITY_Community 273|Community 273]]
- [[_COMMUNITY_Community 274|Community 274]]
- [[_COMMUNITY_Community 275|Community 275]]
- [[_COMMUNITY_Community 276|Community 276]]
- [[_COMMUNITY_Community 277|Community 277]]
- [[_COMMUNITY_Community 278|Community 278]]
- [[_COMMUNITY_Community 279|Community 279]]
- [[_COMMUNITY_Community 280|Community 280]]
- [[_COMMUNITY_Community 281|Community 281]]
- [[_COMMUNITY_Community 282|Community 282]]
- [[_COMMUNITY_Community 283|Community 283]]
- [[_COMMUNITY_Community 284|Community 284]]
- [[_COMMUNITY_Community 285|Community 285]]
- [[_COMMUNITY_Community 286|Community 286]]
- [[_COMMUNITY_Community 287|Community 287]]
- [[_COMMUNITY_Community 288|Community 288]]
- [[_COMMUNITY_Community 289|Community 289]]
- [[_COMMUNITY_Community 290|Community 290]]
- [[_COMMUNITY_Community 291|Community 291]]
- [[_COMMUNITY_Community 292|Community 292]]
- [[_COMMUNITY_Community 293|Community 293]]
- [[_COMMUNITY_Community 294|Community 294]]
- [[_COMMUNITY_Community 295|Community 295]]
- [[_COMMUNITY_Community 296|Community 296]]
- [[_COMMUNITY_Community 297|Community 297]]
- [[_COMMUNITY_Community 298|Community 298]]
- [[_COMMUNITY_Community 299|Community 299]]
- [[_COMMUNITY_Community 300|Community 300]]
- [[_COMMUNITY_Community 301|Community 301]]
- [[_COMMUNITY_Community 302|Community 302]]
- [[_COMMUNITY_Community 303|Community 303]]
- [[_COMMUNITY_Community 304|Community 304]]
- [[_COMMUNITY_Community 305|Community 305]]
- [[_COMMUNITY_Community 306|Community 306]]
- [[_COMMUNITY_Community 307|Community 307]]
- [[_COMMUNITY_Community 308|Community 308]]
- [[_COMMUNITY_Community 309|Community 309]]
- [[_COMMUNITY_Community 310|Community 310]]
- [[_COMMUNITY_Community 311|Community 311]]
- [[_COMMUNITY_Community 312|Community 312]]
- [[_COMMUNITY_Community 313|Community 313]]
- [[_COMMUNITY_Community 314|Community 314]]
- [[_COMMUNITY_Community 315|Community 315]]
- [[_COMMUNITY_Community 316|Community 316]]
- [[_COMMUNITY_Community 317|Community 317]]
- [[_COMMUNITY_Community 318|Community 318]]
- [[_COMMUNITY_Community 319|Community 319]]
- [[_COMMUNITY_Community 320|Community 320]]
- [[_COMMUNITY_Community 321|Community 321]]
- [[_COMMUNITY_Community 322|Community 322]]
- [[_COMMUNITY_Community 323|Community 323]]
- [[_COMMUNITY_Community 324|Community 324]]
- [[_COMMUNITY_Community 325|Community 325]]
- [[_COMMUNITY_Community 326|Community 326]]
- [[_COMMUNITY_Community 327|Community 327]]
- [[_COMMUNITY_Community 328|Community 328]]
- [[_COMMUNITY_Community 329|Community 329]]
- [[_COMMUNITY_Community 330|Community 330]]
- [[_COMMUNITY_Community 331|Community 331]]
- [[_COMMUNITY_Community 332|Community 332]]
- [[_COMMUNITY_Community 333|Community 333]]
- [[_COMMUNITY_Community 334|Community 334]]
- [[_COMMUNITY_Community 335|Community 335]]
- [[_COMMUNITY_Community 336|Community 336]]
- [[_COMMUNITY_Community 337|Community 337]]
- [[_COMMUNITY_Community 338|Community 338]]
- [[_COMMUNITY_Community 339|Community 339]]
- [[_COMMUNITY_Community 340|Community 340]]
- [[_COMMUNITY_Community 341|Community 341]]
- [[_COMMUNITY_Community 342|Community 342]]
- [[_COMMUNITY_Community 343|Community 343]]
- [[_COMMUNITY_Community 344|Community 344]]
- [[_COMMUNITY_Community 345|Community 345]]
- [[_COMMUNITY_Community 346|Community 346]]
- [[_COMMUNITY_Community 347|Community 347]]
- [[_COMMUNITY_Community 348|Community 348]]
- [[_COMMUNITY_Community 349|Community 349]]
- [[_COMMUNITY_Community 350|Community 350]]
- [[_COMMUNITY_Community 351|Community 351]]
- [[_COMMUNITY_Community 352|Community 352]]
- [[_COMMUNITY_Community 353|Community 353]]
- [[_COMMUNITY_Community 354|Community 354]]
- [[_COMMUNITY_Community 355|Community 355]]
- [[_COMMUNITY_Community 356|Community 356]]
- [[_COMMUNITY_Community 357|Community 357]]
- [[_COMMUNITY_Community 358|Community 358]]
- [[_COMMUNITY_Community 359|Community 359]]
- [[_COMMUNITY_Community 360|Community 360]]
- [[_COMMUNITY_Community 361|Community 361]]
- [[_COMMUNITY_Community 362|Community 362]]
- [[_COMMUNITY_Community 363|Community 363]]
- [[_COMMUNITY_Community 364|Community 364]]
- [[_COMMUNITY_Community 365|Community 365]]
- [[_COMMUNITY_Community 366|Community 366]]
- [[_COMMUNITY_Community 367|Community 367]]
- [[_COMMUNITY_Community 368|Community 368]]
- [[_COMMUNITY_Community 369|Community 369]]
- [[_COMMUNITY_Community 370|Community 370]]
- [[_COMMUNITY_Community 371|Community 371]]
- [[_COMMUNITY_Community 372|Community 372]]
- [[_COMMUNITY_Community 373|Community 373]]
- [[_COMMUNITY_Community 374|Community 374]]
- [[_COMMUNITY_Community 375|Community 375]]
- [[_COMMUNITY_Community 376|Community 376]]
- [[_COMMUNITY_Community 377|Community 377]]
- [[_COMMUNITY_Community 378|Community 378]]
- [[_COMMUNITY_Community 379|Community 379]]
- [[_COMMUNITY_Community 380|Community 380]]
- [[_COMMUNITY_Community 381|Community 381]]
- [[_COMMUNITY_Community 382|Community 382]]
- [[_COMMUNITY_Community 383|Community 383]]
- [[_COMMUNITY_Community 384|Community 384]]
- [[_COMMUNITY_Community 385|Community 385]]
- [[_COMMUNITY_Community 386|Community 386]]
- [[_COMMUNITY_Community 387|Community 387]]
- [[_COMMUNITY_Community 388|Community 388]]
- [[_COMMUNITY_Community 389|Community 389]]
- [[_COMMUNITY_Community 390|Community 390]]
- [[_COMMUNITY_Community 391|Community 391]]
- [[_COMMUNITY_Community 392|Community 392]]
- [[_COMMUNITY_Community 393|Community 393]]
- [[_COMMUNITY_Community 394|Community 394]]
- [[_COMMUNITY_Community 395|Community 395]]
- [[_COMMUNITY_Community 396|Community 396]]
- [[_COMMUNITY_Community 397|Community 397]]
- [[_COMMUNITY_Community 398|Community 398]]
- [[_COMMUNITY_Community 399|Community 399]]
- [[_COMMUNITY_Community 400|Community 400]]
- [[_COMMUNITY_Community 401|Community 401]]
- [[_COMMUNITY_Community 402|Community 402]]
- [[_COMMUNITY_Community 403|Community 403]]
- [[_COMMUNITY_Community 404|Community 404]]
- [[_COMMUNITY_Community 405|Community 405]]
- [[_COMMUNITY_Community 406|Community 406]]
- [[_COMMUNITY_Community 407|Community 407]]
- [[_COMMUNITY_Community 408|Community 408]]
- [[_COMMUNITY_Community 409|Community 409]]
- [[_COMMUNITY_Community 410|Community 410]]
- [[_COMMUNITY_Community 411|Community 411]]
- [[_COMMUNITY_Community 412|Community 412]]
- [[_COMMUNITY_Community 413|Community 413]]
- [[_COMMUNITY_Community 414|Community 414]]
- [[_COMMUNITY_Community 415|Community 415]]
- [[_COMMUNITY_Community 416|Community 416]]
- [[_COMMUNITY_Community 417|Community 417]]
- [[_COMMUNITY_Community 418|Community 418]]
- [[_COMMUNITY_Community 419|Community 419]]
- [[_COMMUNITY_Community 420|Community 420]]
- [[_COMMUNITY_Community 421|Community 421]]
- [[_COMMUNITY_Community 422|Community 422]]
- [[_COMMUNITY_Community 423|Community 423]]
- [[_COMMUNITY_Community 424|Community 424]]
- [[_COMMUNITY_Community 425|Community 425]]
- [[_COMMUNITY_Community 426|Community 426]]
- [[_COMMUNITY_Community 427|Community 427]]
- [[_COMMUNITY_Community 428|Community 428]]
- [[_COMMUNITY_Community 429|Community 429]]
- [[_COMMUNITY_Community 430|Community 430]]
- [[_COMMUNITY_Community 431|Community 431]]
- [[_COMMUNITY_Community 432|Community 432]]
- [[_COMMUNITY_Community 433|Community 433]]
- [[_COMMUNITY_Community 434|Community 434]]
- [[_COMMUNITY_Community 435|Community 435]]
- [[_COMMUNITY_Community 436|Community 436]]
- [[_COMMUNITY_Community 437|Community 437]]
- [[_COMMUNITY_Community 438|Community 438]]
- [[_COMMUNITY_Community 439|Community 439]]
- [[_COMMUNITY_Community 440|Community 440]]
- [[_COMMUNITY_Community 441|Community 441]]
- [[_COMMUNITY_Community 442|Community 442]]
- [[_COMMUNITY_Community 443|Community 443]]
- [[_COMMUNITY_Community 444|Community 444]]
- [[_COMMUNITY_Community 445|Community 445]]
- [[_COMMUNITY_Community 446|Community 446]]
- [[_COMMUNITY_Community 447|Community 447]]
- [[_COMMUNITY_Community 448|Community 448]]
- [[_COMMUNITY_Community 449|Community 449]]
- [[_COMMUNITY_Community 450|Community 450]]
- [[_COMMUNITY_Community 451|Community 451]]
- [[_COMMUNITY_Community 452|Community 452]]
- [[_COMMUNITY_Community 453|Community 453]]
- [[_COMMUNITY_Community 454|Community 454]]
- [[_COMMUNITY_Community 455|Community 455]]
- [[_COMMUNITY_Community 456|Community 456]]
- [[_COMMUNITY_Community 457|Community 457]]
- [[_COMMUNITY_Community 458|Community 458]]
- [[_COMMUNITY_Community 459|Community 459]]
- [[_COMMUNITY_Community 460|Community 460]]
- [[_COMMUNITY_Community 461|Community 461]]
- [[_COMMUNITY_Community 462|Community 462]]
- [[_COMMUNITY_Community 463|Community 463]]
- [[_COMMUNITY_Community 464|Community 464]]
- [[_COMMUNITY_Community 465|Community 465]]
- [[_COMMUNITY_Community 466|Community 466]]
- [[_COMMUNITY_Community 467|Community 467]]
- [[_COMMUNITY_Community 468|Community 468]]
- [[_COMMUNITY_Community 469|Community 469]]
- [[_COMMUNITY_Community 470|Community 470]]
- [[_COMMUNITY_Community 471|Community 471]]
- [[_COMMUNITY_Community 472|Community 472]]
- [[_COMMUNITY_Community 473|Community 473]]
- [[_COMMUNITY_Community 474|Community 474]]
- [[_COMMUNITY_Community 475|Community 475]]
- [[_COMMUNITY_Community 476|Community 476]]
- [[_COMMUNITY_Community 477|Community 477]]
- [[_COMMUNITY_Community 478|Community 478]]
- [[_COMMUNITY_Community 479|Community 479]]
- [[_COMMUNITY_Community 480|Community 480]]
- [[_COMMUNITY_Community 481|Community 481]]
- [[_COMMUNITY_Community 482|Community 482]]
- [[_COMMUNITY_Community 483|Community 483]]
- [[_COMMUNITY_Community 484|Community 484]]
- [[_COMMUNITY_Community 485|Community 485]]
- [[_COMMUNITY_Community 486|Community 486]]
- [[_COMMUNITY_Community 487|Community 487]]
- [[_COMMUNITY_Community 488|Community 488]]
- [[_COMMUNITY_Community 489|Community 489]]
- [[_COMMUNITY_Community 490|Community 490]]
- [[_COMMUNITY_Community 491|Community 491]]
- [[_COMMUNITY_Community 492|Community 492]]
- [[_COMMUNITY_Community 493|Community 493]]
- [[_COMMUNITY_Community 494|Community 494]]
- [[_COMMUNITY_Community 495|Community 495]]
- [[_COMMUNITY_Community 496|Community 496]]
- [[_COMMUNITY_Community 497|Community 497]]
- [[_COMMUNITY_Community 498|Community 498]]
- [[_COMMUNITY_Community 499|Community 499]]
- [[_COMMUNITY_Community 500|Community 500]]
- [[_COMMUNITY_Community 501|Community 501]]
- [[_COMMUNITY_Community 502|Community 502]]
- [[_COMMUNITY_Community 503|Community 503]]
- [[_COMMUNITY_Community 504|Community 504]]
- [[_COMMUNITY_Community 505|Community 505]]
- [[_COMMUNITY_Community 506|Community 506]]
- [[_COMMUNITY_Community 507|Community 507]]
- [[_COMMUNITY_Community 508|Community 508]]
- [[_COMMUNITY_Community 509|Community 509]]
- [[_COMMUNITY_Community 510|Community 510]]
- [[_COMMUNITY_Community 511|Community 511]]
- [[_COMMUNITY_Community 512|Community 512]]
- [[_COMMUNITY_Community 513|Community 513]]
- [[_COMMUNITY_Community 514|Community 514]]
- [[_COMMUNITY_Community 516|Community 516]]
- [[_COMMUNITY_Community 518|Community 518]]
- [[_COMMUNITY_Community 519|Community 519]]
- [[_COMMUNITY_Community 520|Community 520]]
- [[_COMMUNITY_Community 521|Community 521]]
- [[_COMMUNITY_Community 522|Community 522]]
- [[_COMMUNITY_Community 524|Community 524]]
- [[_COMMUNITY_Community 525|Community 525]]
- [[_COMMUNITY_Community 526|Community 526]]
- [[_COMMUNITY_Community 527|Community 527]]
- [[_COMMUNITY_Community 530|Community 530]]
- [[_COMMUNITY_Community 533|Community 533]]
- [[_COMMUNITY_Community 534|Community 534]]
- [[_COMMUNITY_Community 535|Community 535]]
- [[_COMMUNITY_Community 536|Community 536]]
- [[_COMMUNITY_Community 537|Community 537]]
- [[_COMMUNITY_Community 538|Community 538]]
- [[_COMMUNITY_Community 539|Community 539]]
- [[_COMMUNITY_Community 540|Community 540]]
- [[_COMMUNITY_Community 542|Community 542]]
- [[_COMMUNITY_Community 543|Community 543]]
- [[_COMMUNITY_Community 544|Community 544]]
- [[_COMMUNITY_Community 545|Community 545]]
- [[_COMMUNITY_Community 546|Community 546]]
- [[_COMMUNITY_Community 547|Community 547]]
- [[_COMMUNITY_Community 548|Community 548]]
- [[_COMMUNITY_Community 549|Community 549]]

## God Nodes (most connected - your core abstractions)
1. `checkRateLimit()` - 240 edges
2. `audits` - 68 edges
3. `audits` - 67 edges
4. `audits` - 67 edges
5. `score` - 66 edges
6. `scoreDisplayMode` - 66 edges
7. `useToast()` - 66 edges
8. `score` - 65 edges
9. `scoreDisplayMode` - 65 edges
10. `score` - 65 edges

## Surprising Connections (you probably didn't know these)
- `Palex Logo — teal wordmark 'Palex', orange tagline 'Improving technologies / Improving lives'` --illustrates--> `Brand Tokens — TEAL=#00A99D, ORANGE=#F7941D (never hardcode hex)`  [INFERRED]
  public/logo-palex.png → AGENTS.md
- `TECHNICAL.md — Complete Technical Reference Document` --extends--> `Palex Platform — Guia del Proyecto`  [INFERRED]
  TECHNICAL.md → AGENTS.md
- `Quality — Lighthouse 100/100/96/100, Playwright 18 E2E, WCAG AA` --validates--> `Design System — Accessible & Ethical Style (WCAG, 44px touch, skip links)`  [INFERRED]
  TECHNICAL.md → design-system/palex-platform/MASTER.md
- `Palex Logo — teal wordmark 'Palex', orange tagline 'Improving technologies / Improving lives'` --represents--> `Palex Platform — Hospital Project Management SaaS (internal tool)`  [EXTRACTED]
  public/logo-palex.png → AGENTS.md
- `MaterialDrawer()` --calls--> `useToast()`  [INFERRED]
  src/app/(dashboard)/admin/hardware/page.tsx → src/components/Toast.tsx

## Import Cycles
- 1-file cycle: `src/app/(dashboard)/visitas/[id]/page.tsx -> src/app/(dashboard)/visitas/[id]/page.tsx`

## Communities (542 total, 15 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.04
Nodes (64): DELETE(), GET(), GET(), POST(), GET(), POST(), DELETE(), GET() (+56 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (71): DELETE(), GET(), GET(), POST(), PATCH(), POST(), PATCH(), DELETE() (+63 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (109): description, id, score, scoreDisplayMode, title, description, id, score (+101 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (115): description, id, score, scoreDisplayMode, title, description, id, score (+107 more)

### Community 4 - "Community 4"
Cohesion: 0.18
Nodes (29): details, details, details, details, details, details, details, details (+21 more)

### Community 5 - "Community 5"
Cohesion: 0.13
Nodes (35): details, details, details, details, details, details, details, details (+27 more)

### Community 6 - "Community 6"
Cohesion: 0.16
Nodes (31): description, details, id, score, scoreDisplayMode, title, details, details (+23 more)

### Community 7 - "Community 7"
Cohesion: 0.06
Nodes (18): GET(), rangoAnterior(), PATCH(), GET(), GET(), NotaPatch, activeCache, { auth, handlers, signIn, signOut } (+10 more)

### Community 8 - "Community 8"
Cohesion: 0.07
Nodes (27): mainthread-work-breakdown, redirects, unsized-images, description, errorMessage, errorStack, guidanceLevel, id (+19 more)

### Community 9 - "Community 9"
Cohesion: 0.07
Nodes (27): redirects, unsized-images, user-timings, description, errorMessage, errorStack, guidanceLevel, id (+19 more)

### Community 10 - "Community 10"
Cohesion: 0.07
Nodes (27): mainthread-work-breakdown, redirects, unsized-images, description, errorMessage, errorStack, guidanceLevel, id (+19 more)

### Community 11 - "Community 11"
Cohesion: 0.07
Nodes (27): mainthread-work-breakdown, redirects, unsized-images, description, errorMessage, errorStack, guidanceLevel, id (+19 more)

### Community 12 - "Community 12"
Cohesion: 0.06
Nodes (42): bool, BM25, detect_domain(), _load_csv(), Lowercase, split, remove punctuation, filter short words, Build BM25 index from documents, Score all documents against query, Load CSV and return list of dicts (+34 more)

### Community 13 - "Community 13"
Cohesion: 0.03
Nodes (63): ALL_TIPOS, CATEGORIAS, ContactoItem, dateGroupLabel(), EQUIPOS, ESTADOS, Evento, FormAction (+55 more)

### Community 14 - "Community 14"
Cohesion: 0.08
Nodes (19): ACCION_CFG, ActividadPage(), ALL_ENTIDADES, DayGroup, ENTIDAD_CFG, ENTIDAD_ICONS, groupByDay(), LogEntry (+11 more)

### Community 15 - "Community 15"
Cohesion: 0.05
Nodes (30): useToast(), AlertasTab(), AsignarUnidadModal(), CAT_FORM_EMPTY, CatTiposModal(), diasDesde(), EditUnidadDrawer(), fmtAntiguedad() (+22 more)

### Community 16 - "Community 16"
Cohesion: 0.06
Nodes (43): ProyectoModulo, Adjunto, Contacto, ContactoPivot, EntradaTimeline, ESTADO_COLOR, ESTADO_LABEL, Fase (+35 more)

### Community 17 - "Community 17"
Cohesion: 0.14
Nodes (18): fetchTime, finalDisplayedUrl, finalUrl, gatherMode, lighthouseVersion, mainDocumentUrl, requestedUrl, runWarnings (+10 more)

### Community 18 - "Community 18"
Cohesion: 0.04
Nodes (57): interactive, max-potential-fid, metrics, network-rtt, network-server-latency, redirects, speed-index, description (+49 more)

### Community 19 - "Community 19"
Cohesion: 0.11
Nodes (24): POST(), GET(), POST(), canAccessVisita(), generarCodigo(), GET(), POST(), logActividad() (+16 more)

### Community 20 - "Community 20"
Cohesion: 0.05
Nodes (28): AlertaItem, AlertasPanel(), IncSla, PRIO_COLOR, SlaAlertasWidget(), agruparPorMes(), agruparPrevisionPorMes(), calcTrend() (+20 more)

### Community 21 - "Community 21"
Cohesion: 0.05
Nodes (41): 1. Accessibility (CRITICAL), 2. Touch & Interaction (CRITICAL), 3. Performance (HIGH), 4. Layout & Responsive (HIGH), 5. Typography & Color (MEDIUM), 6. Animation (MEDIUM), 7. Style Selection (MEDIUM), 8. Charts & Data (LOW) (+33 more)

### Community 22 - "Community 22"
Cohesion: 0.04
Nodes (11): Props, Props, Props, SignaturePad, Props, Props, Props, Props (+3 more)

### Community 23 - "Community 23"
Cohesion: 0.08
Nodes (13): CAT_LABEL, PERIODO_OPTIONS, PRIO_COLOR, PRIO_LABEL, StatsData, TecnicoStat, cn(), Skeleton() (+5 more)

### Community 24 - "Community 24"
Cohesion: 0.04
Nodes (57): interactive, max-potential-fid, metrics, network-rtt, network-server-latency, redirects, speed-index, description (+49 more)

### Community 25 - "Community 25"
Cohesion: 0.09
Nodes (28): axe-core, environment, benchmarkIndex, credits, hostUserAgent, networkUserAgent, fetchTime, finalDisplayedUrl (+20 more)

### Community 26 - "Community 26"
Cohesion: 0.06
Nodes (24): GET(), checkRateLimitByKey(), checkRateLimitByKeyAsync(), lastCleanup, maybePurge(), memIncrement(), memStore, RateLimitEntry (+16 more)

### Community 27 - "Community 27"
Cohesion: 0.07
Nodes (15): CatalogoCard(), CatalogoItem, CatalogoTab(), COLORES_PRESET, ESTADO_INFO, FORM_EMPTY, ringStyle, TiposModal() (+7 more)

### Community 28 - "Community 28"
Cohesion: 0.09
Nodes (20): TagItem, TagPills(), TagSelector(), TagSelectorProps, fechaRel(), ESTADO_COLOR, ESTADO_LABEL, IconChevronRight() (+12 more)

### Community 29 - "Community 29"
Cohesion: 0.22
Nodes (9): user-timings, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 30 - "Community 30"
Cohesion: 0.14
Nodes (27): getConsumoMensual(), getCorrelaciones(), getDiaSemana(), getForecast(), getFranjaHoraria(), getIndicadores(), CAUSAS, DESCS (+19 more)

### Community 31 - "Community 31"
Cohesion: 0.11
Nodes (11): Toast, ToastContext, ToastContextValue, ToastType, ModuloInfo, ESTADO_MODULO_COLOR, ESTADO_MODULO_LABEL, EstadoModulo (+3 more)

### Community 32 - "Community 32"
Cohesion: 0.05
Nodes (24): TipoFavorito, useFavoritos(), HospitalesPage(), TIPO_ICON, Vista, ESTADO_COLOR, ESTADO_LABEL, ESTADOS (+16 more)

### Community 33 - "Community 33"
Cohesion: 0.08
Nodes (12): avatarColor(), IconProps, isRol(), Rol, ROL_CONFIG, RolConfig, ROLES, RolPill() (+4 more)

### Community 34 - "Community 34"
Cohesion: 0.11
Nodes (20): CATEGORIAS, EQUIPOS, EQUIPOS_MAP, ESTADOS, fmtMin(), getEstadoStyle(), getPrioridadStyle(), Hospital (+12 more)

### Community 35 - "Community 35"
Cohesion: 0.05
Nodes (47): layout-shifts, non-composited-animations, server-response-time, unsized-images, user-timings, description, guidanceLevel, id (+39 more)

### Community 36 - "Community 36"
Cohesion: 0.10
Nodes (13): EVENTOS, generarDatos(), Hospital, PeriodoKey, PERIODOS, PuntoTemporal, SEASONAL_MULT, seededRand() (+5 more)

### Community 37 - "Community 37"
Cohesion: 0.07
Nodes (21): ESTADO_MODULO_COLOR, ESTADO_MODULO_LABEL, EstadoModulo, ModuloInlab, ProyectoDetalle(), TabAdjuntos, TabCockpit, TabContactos (+13 more)

### Community 38 - "Community 38"
Cohesion: 0.08
Nodes (24): dependencies, @auth/prisma-adapter, bcryptjs, @dnd-kit/core, @dnd-kit/sortable, @dnd-kit/utilities, next, next-auth (+16 more)

### Community 39 - "Community 39"
Cohesion: 0.05
Nodes (47): layout-shifts, non-composited-animations, server-response-time, unsized-images, user-timings, description, guidanceLevel, id (+39 more)

### Community 40 - "Community 40"
Cohesion: 0.12
Nodes (19): avColor(), Comentario, fechaRel(), Props, extractMentionIds(), MentionInput(), Props, ROL_COLORS (+11 more)

### Community 41 - "Community 41"
Cohesion: 0.15
Nodes (10): PAGE_SIZES, Props, SortDir, SortKey, TabExplorador(), TuboKey, Props, TabIndicadores() (+2 more)

### Community 42 - "Community 42"
Cohesion: 0.08
Nodes (21): CentroGrupo, Contacto, CONTACTO_EMPTY, ESTADO_COLOR, ESTADO_LABEL, FaseResumen, PROY_ESTADO_COLOR, PROY_ESTADO_LABEL (+13 more)

### Community 43 - "Community 43"
Cohesion: 0.24
Nodes (19): OfflineIndicator(), SaveStatus, useOfflineSync(), UseOfflineSyncOptions, useOnlineStatus(), clearSyncQueue(), deleteDraft(), DraftVisita (+11 more)

### Community 44 - "Community 44"
Cohesion: 0.07
Nodes (17): Contacto, FILTROS_FECHA, formatDuration(), Hospital, Llamada, RESULTADOS, IconChevronDown(), IconPhone() (+9 more)

### Community 45 - "Community 45"
Cohesion: 0.08
Nodes (31): Config, ConfiguracionPage(), ModuloItem, ModulosInlabSection(), NIVEL_COLOR, NIVEL_LABEL, PlantillasSection(), REGLA_LABEL (+23 more)

### Community 46 - "Community 46"
Cohesion: 0.10
Nodes (19): compilerOptions, allowJs, esModuleInterop, incremental, isolatedModules, jsx, lib, module (+11 more)

### Community 47 - "Community 47"
Cohesion: 0.17
Nodes (4): useTheme(), Notificacion, Resultado, TopBar()

### Community 48 - "Community 48"
Cohesion: 0.11
Nodes (16): EQUIPOS, ESTADO_EQUIPO, EstadoEquipo, Props, SEV_CONFIG, Alerta, ConsumoParams, DiaSemanaHeatmap (+8 more)

### Community 49 - "Community 49"
Cohesion: 0.07
Nodes (22): CargaData, CargaTrabajoPage(), diasEnMes(), MESES, ROL_COLOR, ROL_LABEL, UsuarioCarga, DatosPage() (+14 more)

### Community 50 - "Community 50"
Cohesion: 0.23
Nodes (14): getKpis(), getLecturasTemperatura(), getNeveras(), getTendencias(), generarAlertasTransporte(), generarIncidenciasTransporte(), generarKpis(), generarLecturasTemperatura() (+6 more)

### Community 51 - "Community 51"
Cohesion: 0.11
Nodes (6): avatarColor(), ConfigApp, PerfilData, PerfilPage(), ROL_CONFIG, IconCheckCircle()

### Community 52 - "Community 52"
Cohesion: 0.14
Nodes (13): ESTADO_COLOR, ESTADO_LABEL, TabFlota(), TIPO_LABEL, TabMapa(), TransporteMapaLeaflet, getRutas(), generarRutas() (+5 more)

### Community 53 - "Community 53"
Cohesion: 0.17
Nodes (17): PrintView, FieldType, FORM_SCHEMA, FormField, FormSection, getSections(), initFormData(), calcProgress() (+9 more)

### Community 54 - "Community 54"
Cohesion: 0.12
Nodes (16): Additional Forbidden Patterns, Anti-Patterns (Do NOT Use), Buttons, Cards, Color Palette, Component Specs, Design System Master File, Global Rules (+8 more)

### Community 55 - "Community 55"
Cohesion: 0.12
Nodes (41): details, details, details, details, details, initiators, networkStartTimeTs, tasks (+33 more)

### Community 56 - "Community 56"
Cohesion: 0.14
Nodes (6): ComparadorData, delta(), fmtNum(), MetricCard(), PERIODOS, PeriodStats

### Community 57 - "Community 57"
Cohesion: 0.12
Nodes (15): axe-core, environment, benchmarkIndex, credits, hostUserAgent, networkUserAgent, fetchTime, finalDisplayedUrl (+7 more)

### Community 58 - "Community 58"
Cohesion: 0.12
Nodes (15): axe-core, environment, benchmarkIndex, credits, hostUserAgent, networkUserAgent, fetchTime, finalDisplayedUrl (+7 more)

### Community 59 - "Community 59"
Cohesion: 0.14
Nodes (12): CardContent(), ESTADO_COLOR, ESTADO_LABEL, estadoEfectivo(), ESTADOS, FaseResumen, fmtFecha(), Hospital (+4 more)

### Community 60 - "Community 60"
Cohesion: 0.12
Nodes (15): ShareData, ContactoShare, ESTADO_COLOR, ESTADO_LABEL, FASE_COLOR, FaseShare, fmt(), HitoShare (+7 more)

### Community 61 - "Community 61"
Cohesion: 0.15
Nodes (9): CalendarioPage(), DIAS_SEMANA, diasEnMes(), ESTADO_COLOR, ESTADO_LABEL, fechaKey(), MESES, primerDiaMes() (+1 more)

### Community 62 - "Community 62"
Cohesion: 0.15
Nodes (11): TempChart(), TendenciaChart(), TabTendencias(), LecturaTemperatura, ParadaRuta, PERIODOS_TRANSPORTE, TendenciaPunto, EstadoIncidencia (+3 more)

### Community 63 - "Community 63"
Cohesion: 0.12
Nodes (6): Accion, ACCIONES_BASE, CommandPalette(), KeyboardShortcutsProvider(), useKeyboardShortcuts(), TIPO_RESULTADO_COLOR

### Community 64 - "Community 64"
Cohesion: 0.14
Nodes (8): AreaChart(), DonutChart(), TuboKey, Props, TuboKey, EVENTOS, ConsumoMensual, ForecastPunto

### Community 65 - "Community 65"
Cohesion: 0.16
Nodes (13): ForecastChart(), calcR2(), EJE_CONFIG, EjeX, Props, ScatterPlot(), TabCorrelaciones(), ANOMALIAS (+5 more)

### Community 66 - "Community 66"
Cohesion: 0.25
Nodes (4): ESTADO_RUTA_COLOR, ESTADO_RUTA_LABEL, AlertaTransporte, KpiResumen

### Community 67 - "Community 67"
Cohesion: 0.16
Nodes (9): garantiaStatus(), HardwarePassportPage(), INC_ESTADO_COLOR, PRIO_COLOR, PRIO_LABEL, Unidad, ESTADO_COLOR, ESTADO_LABEL (+1 more)

### Community 68 - "Community 68"
Cohesion: 0.29
Nodes (10): getActiveUsers(), heartbeat(), lastCleanup, leave(), maybePurge(), memStore, PresenceEntry, getRedis() (+2 more)

### Community 69 - "Community 69"
Cohesion: 0.15
Nodes (12): APIs — notas importantes, Checklist obligatorio antes de cada deploy a produccion, Completado (sesion 2026-05-22), Estructura de ficheros clave, Modulo Hardware en la plataforma, Nomenclatura hardware Palex (APRENDER — usar siempre estos nombres exactos), PALEX PLATFORM — Contexto de Proyecto para IA, Patrones y convenciones de codigo (+4 more)

### Community 70 - "Community 70"
Cohesion: 0.15
Nodes (3): Action, QuickActionsFAB(), useContextActions()

### Community 71 - "Community 71"
Cohesion: 0.15
Nodes (13): scripts, build, db:generate, db:migrate, db:push, db:seed, db:seed-modulos, db:studio (+5 more)

### Community 72 - "Community 72"
Cohesion: 0.15
Nodes (12): background_color, categories, description, display, icons, lang, name, orientation (+4 more)

### Community 73 - "Community 73"
Cohesion: 0.18
Nodes (12): Implemented Features Summary (Sprint 1-18), Timeline Global de Actividad — /actividad, feed estilo GitHub, filtros por entidad, Check-in/Check-out Hospitales — CheckinHospital model, idempotent, live counter 60s, Pasaporte Hardware — /share/hardware/[id], public no-auth page, CSP exempt, iCal Feed — HMAC-SHA256 token auth, visitas + recordatorios + hitos, KanbanView — @dnd-kit dynamic import, isolated DnD for proyectos, @Menciones System — MentionInput + MentionText, stored as @[userId:Nombre], pills, Notas del Equipo — NotaEquipo model (notas_equipo), CRUD, @menciones, acceso todos los roles (+4 more)

### Community 74 - "Community 74"
Cohesion: 0.12
Nodes (14): BottomNav(), NavTab, tabs, Icons, NAV_GROUPS, NAV_GROUPS_ADMIN, NAV_GROUPS_PROYECTOS, NAV_GROUPS_VENTAS (+6 more)

### Community 75 - "Community 75"
Cohesion: 0.10
Nodes (12): Tarea, ESTADO_FASE, ESTADO_PROY, fmtFecha(), PRIORIDAD_COLOR, progreso(), SlideKPIs(), SlidePortada() (+4 more)

### Community 76 - "Community 76"
Cohesion: 0.11
Nodes (5): ADMIN_STEPS, FINAL_STEP, GENERAL_STEPS, Step, IconHospital()

### Community 77 - "Community 77"
Cohesion: 0.18
Nodes (11): Assistant Rules — simplicity, mobile-first, UI/UX Pro Max, Feature Backlog — Resend email, VAPID push, offline, incidencias, Disabled Modules — CRM Pipeline, Incidencias toggle, Hardware Nomenclature — BC Robo, Zebra MC, Reader RFID, etc., Prisma 7 Critical Rules, Palex Platform — Guia del Proyecto, User Roles — ADMIN, VENTAS, PROYECTOS, TECNICO, URL Routing Rules — Route Groups & Forbidden Paths (+3 more)

### Community 78 - "Community 78"
Cohesion: 0.21
Nodes (8): COORDS_POR_CIUDAD, geoDist(), getCoords(), HospitalMapa, nearestNeighbor(), TIPO_LABELS, ZONE_PALETTE, MapaLeaflet

### Community 79 - "Community 79"
Cohesion: 0.12
Nodes (15): ESTADO_PARADA_COLOR, ESTADO_PARADA_LABEL, ESTADO_RUTA_COLOR, ESTADO_RUTA_LABEL, DESCS_INC, ESTADOS_INC, GRAVEDADES, LABORATORIO (+7 more)

### Community 80 - "Community 80"
Cohesion: 0.17
Nodes (10): ESTADO_ORDER, GRAV_COLOR, GRAV_ORDER, Props, SortDir, SortKey, TIPO_COLOR, Incidencia (+2 more)

### Community 81 - "Community 81"
Cohesion: 0.18
Nodes (9): FORM_EMPTY, HospitalesAdminPage(), TIPO_COLOR, TIPO_LABELS_FULL, Zona, exportarExcel(), exportarExcelSimple(), Hospital (+1 more)

### Community 82 - "Community 82"
Cohesion: 0.20
Nodes (10): Code Conventions — use client first line, brand.ts colors, SVG icons, Brand Tokens — TEAL=#00A99D, ORANGE=#F7941D (never hardcode hex), Palex Platform — Hospital Project Management SaaS (internal tool), Design System — Accessible & Ethical Style (WCAG, 44px touch, skip links), Design System — Anti-Patterns (no neon, no motion-heavy, no emojis as icons), Design System — Color Palette (Primary #0891B2, CTA #22C55E, BG #F0FDFA), Design System MASTER.md — Color palette, typography, spacing, components, Design System — Typography (Figtree headings, Noto Sans body) (+2 more)

### Community 83 - "Community 83"
Cohesion: 0.31
Nodes (10): File Structure — src/app, components, hooks, lib, Graphify Query Rules — query/path/explain before raw grep, checkRateLimit() — God Node (22 edges, rate limiting across all APIs), Graphify Knowledge Graph — 1229 nodes, 1607 edges, 115 communities, god nodes, openDB() — God Node (10 edges, IndexedDB offline), Toast.tsx — God Node (42 edges, global notification system), usePerfil() — SWR hook for role/profile, shared across 10+ pages, dedup 60s, .claude/CLAUDE.md — graphify skill trigger, user email, currentDate (+2 more)

### Community 84 - "Community 84"
Cohesion: 0.20
Nodes (10): NextAuth v5 Critical Rules, Prisma Data Models (27 tables, 15 enums), API Endpoints (~50 REST routes) — hospitales, visitas, proyectos, Auth System — NextAuth v5 split config, JWT revocation, IDOR, Data Models — 27 tables, ER relationships, 15 enums, Feature Modules — Hospitales, Visitas, Proyectos, Hardware, Llamadas, Performance Optimizations — SWR dedup, code splitting, connection pool, Quality — Lighthouse 100/100/96/100, Playwright 18 E2E, WCAG AA (+2 more)

### Community 85 - "Community 85"
Cohesion: 0.15
Nodes (12): eslintConfig, devDependencies, dotenv, eslint, eslint-config-next, @playwright/test, @types/bcryptjs, @types/node (+4 more)

### Community 86 - "Community 86"
Cohesion: 0.24
Nodes (9): diasDesde(), Etapa, ETAPA_COLOR, ETAPA_LABEL, ETAPAS, HistorialEntry, Oportunidad, PipelineFichaPage() (+1 more)

### Community 87 - "Community 87"
Cohesion: 0.28
Nodes (9): Deploy Checklist & Railway Config, Tech Stack (Next.js 14+, Prisma 7, NextAuth v5, Redis, Railway), Next.js 14+ App Router — server/client components, route groups, edge middleware, PWA Offline — IndexedDB + Service Worker, draft auto-save for visitas, Railway Deploy — auto-deploy on git push, prisma db push on start, healthcheck /api/health, Redis Graceful Fallback — Upstash Redis with in-memory fallback if no env vars, README — Next.js project bootstrapped with create-next-app, Architecture — App Router, Route Groups, Edge Middleware, API Routes (+1 more)

### Community 88 - "Community 88"
Cohesion: 0.12
Nodes (41): details, details, details, details, details, initiators, networkStartTimeTs, tasks (+33 more)

### Community 89 - "Community 89"
Cohesion: 0.22
Nodes (6): SEV_COLOR, SEV_LABEL, TIPO_LABEL, Nevera, TipoAlertaTransporte, TIPO_ICON

### Community 90 - "Community 90"
Cohesion: 0.06
Nodes (33): 10. Resumen semanal ADMIN (esfuerzo medio), 11. Ruta optimizada mapa (esfuerzo alto), 12. Check-in/Check-out hospitales (esfuerzo medio), 1. Plantillas de proyecto inteligentes (PRIORIDAD ALTA — esfuerzo bajo), 2. Panel de notas del equipo (PRIORIDAD ALTA — esfuerzo bajo), 3. Timeline global de actividad (PRIORIDAD ALTA — esfuerzo bajo), 4. Scoring de hospitales (PRIORIDAD ALTA — esfuerzo bajo), 5. Briefing matutino automatico (PRIORIDAD ALTA — esfuerzo medio) (+25 more)

### Community 91 - "Community 91"
Cohesion: 0.25
Nodes (8): Admin, Auth y navegacion, Calidad tecnica, CRM / Ventas, Dark mode (globals.css), Funcionalidades implementadas (mayo 2026), Visitas, Hospital

### Community 92 - "Community 92"
Cohesion: 0.22
Nodes (6): ESTADO_LABEL, GRAVEDAD_COLOR, TIPO_LABEL, IncidenciaTransporte, TipoIncidenciaTransporte, ESTADO_COLOR

### Community 93 - "Community 93"
Cohesion: 0.25
Nodes (7): engines, node, name, prisma, seed, private, version

### Community 94 - "Community 94"
Cohesion: 0.16
Nodes (5): NotificationManager(), NotifItem, OnboardingWizard(), PageTransition(), ToastProvider()

### Community 95 - "Community 95"
Cohesion: 0.47
Nodes (6): API Rules — Rate limiting, Zod, IDOR, pagination, Hospital Health Score — 0-100 dynamic (visitas 30pts, proyectos 30pts, HW 20pts, llamadas 20pts, -15pts criticas), Hospital Score API — GET /api/hospitales/[id]/score + batch /api/hospitales/score?ids=, Incidencias Equipo Enum — SERVICIO_TECNICO, APLICACIONES, COMERCIAL, MARKETING, PROYECTOS (5, no AMBOS), SLA Pause Mechanism — slaPausadoEn + slaPausadoMs on PENDIENTE_CLIENTE/PROVEEDOR, Zod v4 Schemas — centralized in schemas.ts, parseBody() helper

### Community 96 - "Community 96"
Cohesion: 0.33
Nodes (6): Roadmap de funcionalidades — analisis mayo 2026, TIER 1 — Productividad diaria (mayor ROI para el usuario), TIER 2 — Colaboracion y visibilidad de equipo, TIER 3 — Datos e informes, TIER 4 — Campo y movilidad, TIER 5 — Diferenciadores premium

### Community 97 - "Community 97"
Cohesion: 0.20
Nodes (8): geistSans, metadata, viewport, ServiceWorkerRegistrar(), Ctx, Theme, ThemeCtx, ThemeProvider()

### Community 98 - "Community 98"
Cohesion: 0.53
Nodes (4): GET(), icalEscape(), toICalDate(), toICalDateTime()

### Community 99 - "Community 99"
Cohesion: 0.07
Nodes (34): first-contentful-paint, largest-contentful-paint, total-blocking-time, scoringOptions, description, displayValue, id, numericUnit (+26 more)

### Community 100 - "Community 100"
Cohesion: 0.07
Nodes (34): first-contentful-paint, largest-contentful-paint, total-blocking-time, scoringOptions, description, displayValue, id, numericUnit (+26 more)

### Community 101 - "Community 101"
Cohesion: 0.33
Nodes (6): axe-core, environment, benchmarkIndex, credits, hostUserAgent, networkUserAgent

### Community 102 - "Community 102"
Cohesion: 0.33
Nodes (6): axe-core, environment, benchmarkIndex, credits, hostUserAgent, networkUserAgent

### Community 104 - "Community 104"
Cohesion: 0.40
Nodes (4): Git en sandbox — NUNCA usar git add / git commit normales, Patrones criticos — sandbox de Cowork (solo relevante en Cowork, no en Claude Code CLI), update-index: usar siempre --add --cacheinfo, Write/Edit tool trunca archivos > ~100 lineas en silencio

### Community 105 - "Community 105"
Cohesion: 0.40
Nodes (5): NextAuth v5 — CRITICO, Prisma — CRITICO, Roles del sistema, Stack tecnico (NO cambiar sin justificacion), URL routing — CRITICO

### Community 106 - "Community 106"
Cohesion: 0.40
Nodes (5): Pendiente inmediato (proxima sesion), Sprint 6 — Quick wins de alto impacto, Sprint 7 — Colaboracion, Sprint 8 — Datos y reportes, Sprint 9 — Calidad tecnica

### Community 107 - "Community 107"
Cohesion: 0.33
Nodes (4): hooks, PreToolUse, SessionStart, Hook SessionStart — inyecta CONTEXT.md y estado del grafo graphify. Ejecutado au

### Community 108 - "Community 108"
Cohesion: 0.08
Nodes (16): GET(), GET(), globalForPrisma, computeHospitalScore(), HospitalScore, GET(), NotaCreate, POST() (+8 more)

### Community 109 - "Community 109"
Cohesion: 0.40
Nodes (3): adapter, db, MODULOS

### Community 110 - "Community 110"
Cohesion: 0.08
Nodes (25): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+17 more)

### Community 112 - "Community 112"
Cohesion: 0.83
Nodes (3): canAccessVisita(), GET(), POST()

### Community 113 - "Community 113"
Cohesion: 0.83
Nodes (3): GET(), getOrCreateConfig(), PATCH()

### Community 114 - "Community 114"
Cohesion: 0.50
Nodes (4): environment, benchmarkIndex, credits, hostUserAgent

### Community 115 - "Community 115"
Cohesion: 0.14
Nodes (14): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-attr (+6 more)

### Community 116 - "Community 116"
Cohesion: 0.50
Nodes (4): environment, benchmarkIndex, credits, hostUserAgent

### Community 117 - "Community 117"
Cohesion: 0.14
Nodes (14): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-attr (+6 more)

### Community 118 - "Community 118"
Cohesion: 0.50
Nodes (4): environment, benchmarkIndex, credits, hostUserAgent

### Community 119 - "Community 119"
Cohesion: 0.14
Nodes (14): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-attr (+6 more)

### Community 121 - "Community 121"
Cohesion: 0.50
Nodes (3): Deploy on Vercel, Getting Started, Learn More

### Community 122 - "Community 122"
Cohesion: 0.50
Nodes (3): JWT, Session, User

### Community 123 - "Community 123"
Cohesion: 0.25
Nodes (8): image-aspect-ratio, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 126 - "Community 126"
Cohesion: 0.10
Nodes (20): exportarCSV(), DraggableCard(), Etapa, ETAPA_COLOR, ETAPA_LABEL, ETAPAS, fmtEuros(), FORM_VACIO (+12 more)

### Community 127 - "Community 127"
Cohesion: 0.22
Nodes (9): mainthread-work-breakdown, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 128 - "Community 128"
Cohesion: 0.25
Nodes (8): image-aspect-ratio, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 129 - "Community 129"
Cohesion: 0.25
Nodes (8): max-potential-fid, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 132 - "Community 132"
Cohesion: 0.25
Nodes (8): network-server-latency, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 133 - "Community 133"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-hidden-focus

### Community 134 - "Community 134"
Cohesion: 0.25
Nodes (8): image-aspect-ratio, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 153 - "Community 153"
Cohesion: 0.10
Nodes (23): network-requests, script-treemap-data, data, nodes, scale, skipSumming, summary, timestamp (+15 more)

### Community 154 - "Community 154"
Cohesion: 0.10
Nodes (23): is-on-https, script-treemap-data, data, nodes, scale, skipSumming, summary, timestamp (+15 more)

### Community 155 - "Community 155"
Cohesion: 0.09
Nodes (22): long-tasks, mainthread-work-breakdown, metricSavings, description, displayValue, guidanceLevel, id, metricSavings (+14 more)

### Community 156 - "Community 156"
Cohesion: 0.09
Nodes (22): long-tasks, mainthread-work-breakdown, metricSavings, description, displayValue, guidanceLevel, id, metricSavings (+14 more)

### Community 157 - "Community 157"
Cohesion: 0.10
Nodes (21): description, id, score, scoreDisplayMode, title, aria-input-field-name, focus-traps, landmark-one-main (+13 more)

### Community 158 - "Community 158"
Cohesion: 0.10
Nodes (20): description, id, score, scoreDisplayMode, title, aria-roles, frame-title, input-button-name (+12 more)

### Community 159 - "Community 159"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, description, id, score (+11 more)

### Community 160 - "Community 160"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, description, id, score (+11 more)

### Community 161 - "Community 161"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, description, id, score (+11 more)

### Community 162 - "Community 162"
Cohesion: 0.11
Nodes (19): image-redundant-alt, input-button-name, table-fake-caption, description, id, score, scoreDisplayMode, title (+11 more)

### Community 163 - "Community 163"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, accesskeys, bypass, heading-order (+11 more)

### Community 164 - "Community 164"
Cohesion: 0.11
Nodes (19): listitem, table-duplicate-name, video-caption, description, description, id, score, scoreDisplayMode (+11 more)

### Community 165 - "Community 165"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, aria-allowed-role, input-image-alt, td-headers-attr (+11 more)

### Community 166 - "Community 166"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, audits, aria-text, label-content-name-mismatch (+11 more)

### Community 167 - "Community 167"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, description, id, score (+11 more)

### Community 168 - "Community 168"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-command-name

### Community 169 - "Community 169"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, aria-input-field-name, bypass, object-alt (+11 more)

### Community 170 - "Community 170"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, description, id, score (+11 more)

### Community 171 - "Community 171"
Cohesion: 0.11
Nodes (19): description, id, score, scoreDisplayMode, title, description, id, score (+11 more)

### Community 172 - "Community 172"
Cohesion: 0.11
Nodes (18): 10. Patrones y convenciones de codigo, 11. Nomenclatura hardware Palex, 12. Deploy a produccion, 13. Reglas del asistente, 1. Stack tecnico (NO cambiar sin justificacion), 3. Modulos desactivados, 4. Estructura de ficheros, 5. Modelos Prisma principales (+10 more)

### Community 173 - "Community 173"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 174 - "Community 174"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 175 - "Community 175"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 176 - "Community 176"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-progressbar-name (+9 more)

### Community 177 - "Community 177"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 178 - "Community 178"
Cohesion: 0.12
Nodes (17): audits, image-size-responsive, redirects-http, description, errorMessage, errorStack, id, score (+9 more)

### Community 179 - "Community 179"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 180 - "Community 180"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 181 - "Community 181"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 182 - "Community 182"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 183 - "Community 183"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-roles (+9 more)

### Community 184 - "Community 184"
Cohesion: 0.12
Nodes (17): audits, image-size-responsive, redirects-http, description, errorMessage, errorStack, id, score (+9 more)

### Community 185 - "Community 185"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 186 - "Community 186"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 187 - "Community 187"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, audits (+9 more)

### Community 188 - "Community 188"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 189 - "Community 189"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-roles (+9 more)

### Community 190 - "Community 190"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 191 - "Community 191"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, audits (+9 more)

### Community 192 - "Community 192"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 193 - "Community 193"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-deprecated-role (+9 more)

### Community 194 - "Community 194"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 195 - "Community 195"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-roles (+9 more)

### Community 196 - "Community 196"
Cohesion: 0.12
Nodes (17): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, description (+9 more)

### Community 197 - "Community 197"
Cohesion: 0.13
Nodes (15): description, id, score, scoreDisplayMode, title, audits, aria-required-attr, document-title (+7 more)

### Community 198 - "Community 198"
Cohesion: 0.13
Nodes (15): 7.10 Busqueda y navegacion, 7.11 Modo campo movil, 7.12 Onboarding, 7.13 Notificaciones, 7.14 Calendario iCal, 7.1 Hospitales, 7.2 Visitas tecnicas, 7.3 Proyectos (+7 more)

### Community 199 - "Community 199"
Cohesion: 0.14
Nodes (14): list, skip-link, td-headers-attr, scoreDisplayMode, description, id, score, scoreDisplayMode (+6 more)

### Community 200 - "Community 200"
Cohesion: 0.14
Nodes (14): dlitem, focus-traps, list, description, id, score, scoreDisplayMode, title (+6 more)

### Community 201 - "Community 201"
Cohesion: 0.14
Nodes (14): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-attr (+6 more)

### Community 202 - "Community 202"
Cohesion: 0.17
Nodes (11): 10. Seguridad, 12. Rendimiento, 14. Comandos de desarrollo, 5. Sistema de autenticacion y roles, Documento Tecnico — Plataforma de Gestion de Proyectos Hospitalarios, Headers HTTP (next.config.ts), NextAuth v5, Optimizaciones implementadas (+3 more)

### Community 203 - "Community 203"
Cohesion: 0.18
Nodes (10): 1. Que es el proyecto, 2. Stack tecnico, 3. Estructura de archivos clave, 4. Roles del sistema, 5. Base de datos — Prisma 7, 6. API Routes principales, 7. URL routing — CRITICO, 8. UI/UX — Reglas criticas (+2 more)

### Community 204 - "Community 204"
Cohesion: 0.20
Nodes (10): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, accesskeys (+2 more)

### Community 205 - "Community 205"
Cohesion: 0.20
Nodes (10): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, accesskeys (+2 more)

### Community 206 - "Community 206"
Cohesion: 0.20
Nodes (9): fetchTime, finalDisplayedUrl, finalUrl, gatherMode, lighthouseVersion, mainDocumentUrl, requestedUrl, runWarnings (+1 more)

### Community 207 - "Community 207"
Cohesion: 0.20
Nodes (10): bootup-time, description, displayValue, guidanceLevel, id, numericUnit, numericValue, score (+2 more)

### Community 208 - "Community 208"
Cohesion: 0.20
Nodes (9): fetchTime, finalDisplayedUrl, finalUrl, gatherMode, lighthouseVersion, mainDocumentUrl, requestedUrl, runWarnings (+1 more)

### Community 209 - "Community 209"
Cohesion: 0.20
Nodes (10): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, accesskeys (+2 more)

### Community 210 - "Community 210"
Cohesion: 0.20
Nodes (10): bootup-time, description, displayValue, guidanceLevel, id, numericUnit, numericValue, score (+2 more)

### Community 211 - "Community 211"
Cohesion: 0.20
Nodes (9): fetchTime, finalDisplayedUrl, finalUrl, gatherMode, lighthouseVersion, mainDocumentUrl, requestedUrl, runWarnings (+1 more)

### Community 212 - "Community 212"
Cohesion: 0.20
Nodes (10): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, accesskeys (+2 more)

### Community 213 - "Community 213"
Cohesion: 0.22
Nodes (4): DIAS, MESES, Props, TabAnalitica()

### Community 214 - "Community 214"
Cohesion: 0.22
Nodes (9): bootup-time, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 215 - "Community 215"
Cohesion: 0.22
Nodes (9): layout-shifts, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 216 - "Community 216"
Cohesion: 0.22
Nodes (9): long-tasks, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 217 - "Community 217"
Cohesion: 0.22
Nodes (9): non-composited-animations, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 218 - "Community 218"
Cohesion: 0.22
Nodes (9): server-response-time, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 219 - "Community 219"
Cohesion: 0.22
Nodes (9): user-timings, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 220 - "Community 220"
Cohesion: 0.22
Nodes (9): bootup-time, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 221 - "Community 221"
Cohesion: 0.22
Nodes (9): layout-shifts, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 222 - "Community 222"
Cohesion: 0.22
Nodes (9): long-tasks, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 223 - "Community 223"
Cohesion: 0.22
Nodes (9): non-composited-animations, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 224 - "Community 224"
Cohesion: 0.22
Nodes (9): server-response-time, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 225 - "Community 225"
Cohesion: 0.22
Nodes (9): user-timings, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 226 - "Community 226"
Cohesion: 0.22
Nodes (9): cumulative-layout-shift, description, displayValue, id, numericUnit, numericValue, score, scoreDisplayMode (+1 more)

### Community 227 - "Community 227"
Cohesion: 0.22
Nodes (9): bootup-time, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 228 - "Community 228"
Cohesion: 0.22
Nodes (9): layout-shifts, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 229 - "Community 229"
Cohesion: 0.22
Nodes (9): long-tasks, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 230 - "Community 230"
Cohesion: 0.22
Nodes (9): non-composited-animations, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 231 - "Community 231"
Cohesion: 0.22
Nodes (9): server-response-time, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 232 - "Community 232"
Cohesion: 0.25
Nodes (8): image-aspect-ratio, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 233 - "Community 233"
Cohesion: 0.22
Nodes (9): cumulative-layout-shift, description, displayValue, id, numericUnit, numericValue, score, scoreDisplayMode (+1 more)

### Community 234 - "Community 234"
Cohesion: 0.22
Nodes (9): bootup-time, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 235 - "Community 235"
Cohesion: 0.22
Nodes (9): layout-shifts, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 236 - "Community 236"
Cohesion: 0.22
Nodes (9): long-tasks, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 237 - "Community 237"
Cohesion: 0.25
Nodes (8): network-requests, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 238 - "Community 238"
Cohesion: 0.22
Nodes (9): non-composited-animations, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 239 - "Community 239"
Cohesion: 0.22
Nodes (9): server-response-time, description, errorMessage, errorStack, guidanceLevel, id, score, scoreDisplayMode (+1 more)

### Community 240 - "Community 240"
Cohesion: 0.22
Nodes (9): 4. Modelo de datos, Diagrama de relaciones, Entidades de negocio, Entidades de proyecto, Enums (15 totales), Hardware, Indices de base de datos (15), Modelos principales (27 tablas) (+1 more)

### Community 241 - "Community 241"
Cohesion: 0.33
Nodes (5): Auditoria profunda — Sprint 20 (2026-07-09), Logica de negocio / Prisma, Prioridad de ejecucion aplicada este sprint, Seguridad API, UX / consistencia frontend

### Community 242 - "Community 242"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-allowed-role

### Community 243 - "Community 243"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-conditional-attr

### Community 244 - "Community 244"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-dialog-name

### Community 245 - "Community 245"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-hidden-focus

### Community 246 - "Community 246"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-input-field-name

### Community 247 - "Community 247"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-meter-name

### Community 248 - "Community 248"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-children

### Community 249 - "Community 249"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-parent

### Community 250 - "Community 250"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-text

### Community 251 - "Community 251"
Cohesion: 0.25
Nodes (8): clickjacking-mitigation, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 252 - "Community 252"
Cohesion: 0.25
Nodes (8): csp-xss, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 253 - "Community 253"
Cohesion: 0.25
Nodes (8): cumulative-layout-shift, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 254 - "Community 254"
Cohesion: 0.25
Nodes (8): deprecations, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 255 - "Community 255"
Cohesion: 0.25
Nodes (8): diagnostics, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 256 - "Community 256"
Cohesion: 0.25
Nodes (8): errors-in-console, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 257 - "Community 257"
Cohesion: 0.25
Nodes (8): final-screenshot, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 258 - "Community 258"
Cohesion: 0.25
Nodes (8): first-contentful-paint, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 259 - "Community 259"
Cohesion: 0.25
Nodes (8): has-hsts, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 260 - "Community 260"
Cohesion: 0.25
Nodes (8): interactive, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 261 - "Community 261"
Cohesion: 0.25
Nodes (8): is-on-https, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 262 - "Community 262"
Cohesion: 0.25
Nodes (8): largest-contentful-paint, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 263 - "Community 263"
Cohesion: 0.25
Nodes (8): main-thread-tasks, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 264 - "Community 264"
Cohesion: 0.25
Nodes (8): max-potential-fid, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 265 - "Community 265"
Cohesion: 0.25
Nodes (8): network-rtt, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 266 - "Community 266"
Cohesion: 0.25
Nodes (8): network-server-latency, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 267 - "Community 267"
Cohesion: 0.25
Nodes (8): origin-isolation, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 268 - "Community 268"
Cohesion: 0.25
Nodes (8): resource-summary, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 269 - "Community 269"
Cohesion: 0.25
Nodes (8): screenshot-thumbnails, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 270 - "Community 270"
Cohesion: 0.25
Nodes (8): script-treemap-data, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 271 - "Community 271"
Cohesion: 0.25
Nodes (8): speed-index, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 272 - "Community 272"
Cohesion: 0.25
Nodes (8): third-party-cookies, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 273 - "Community 273"
Cohesion: 0.25
Nodes (8): total-blocking-time, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 274 - "Community 274"
Cohesion: 0.25
Nodes (8): trusted-types-xss, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 275 - "Community 275"
Cohesion: 0.25
Nodes (8): valid-source-maps, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 276 - "Community 276"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-allowed-role

### Community 277 - "Community 277"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-conditional-attr

### Community 278 - "Community 278"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-dialog-name

### Community 279 - "Community 279"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-hidden-focus

### Community 280 - "Community 280"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-input-field-name

### Community 281 - "Community 281"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-meter-name

### Community 282 - "Community 282"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-children

### Community 283 - "Community 283"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-parent

### Community 284 - "Community 284"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-text

### Community 285 - "Community 285"
Cohesion: 0.25
Nodes (8): clickjacking-mitigation, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 286 - "Community 286"
Cohesion: 0.25
Nodes (8): csp-xss, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 287 - "Community 287"
Cohesion: 0.25
Nodes (8): cumulative-layout-shift, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 288 - "Community 288"
Cohesion: 0.25
Nodes (8): deprecations, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 289 - "Community 289"
Cohesion: 0.25
Nodes (8): diagnostics, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 290 - "Community 290"
Cohesion: 0.25
Nodes (8): errors-in-console, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 291 - "Community 291"
Cohesion: 0.25
Nodes (8): final-screenshot, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 292 - "Community 292"
Cohesion: 0.25
Nodes (8): first-contentful-paint, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 293 - "Community 293"
Cohesion: 0.25
Nodes (8): has-hsts, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 294 - "Community 294"
Cohesion: 0.33
Nodes (6): dlitem, description, id, score, scoreDisplayMode, title

### Community 295 - "Community 295"
Cohesion: 0.25
Nodes (8): interactive, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 296 - "Community 296"
Cohesion: 0.25
Nodes (8): is-on-https, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 297 - "Community 297"
Cohesion: 0.25
Nodes (8): largest-contentful-paint, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 298 - "Community 298"
Cohesion: 0.25
Nodes (8): main-thread-tasks, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 299 - "Community 299"
Cohesion: 0.25
Nodes (8): max-potential-fid, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 300 - "Community 300"
Cohesion: 0.25
Nodes (8): network-rtt, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 301 - "Community 301"
Cohesion: 0.25
Nodes (8): network-server-latency, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 302 - "Community 302"
Cohesion: 0.25
Nodes (8): origin-isolation, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 303 - "Community 303"
Cohesion: 0.25
Nodes (8): resource-summary, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 304 - "Community 304"
Cohesion: 0.25
Nodes (8): screenshot-thumbnails, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 305 - "Community 305"
Cohesion: 0.25
Nodes (8): script-treemap-data, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 306 - "Community 306"
Cohesion: 0.25
Nodes (8): speed-index, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 307 - "Community 307"
Cohesion: 0.25
Nodes (8): third-party-cookies, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 308 - "Community 308"
Cohesion: 0.25
Nodes (8): total-blocking-time, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 309 - "Community 309"
Cohesion: 0.25
Nodes (8): trusted-types-xss, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 310 - "Community 310"
Cohesion: 0.25
Nodes (8): valid-source-maps, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 311 - "Community 311"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-allowed-role

### Community 312 - "Community 312"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-conditional-attr

### Community 313 - "Community 313"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-dialog-name

### Community 314 - "Community 314"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-input-field-name

### Community 315 - "Community 315"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-meter-name

### Community 316 - "Community 316"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-children

### Community 317 - "Community 317"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-parent

### Community 318 - "Community 318"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-text

### Community 319 - "Community 319"
Cohesion: 0.25
Nodes (8): clickjacking-mitigation, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 320 - "Community 320"
Cohesion: 0.25
Nodes (8): csp-xss, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 321 - "Community 321"
Cohesion: 0.25
Nodes (8): cumulative-layout-shift, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 322 - "Community 322"
Cohesion: 0.25
Nodes (8): deprecations, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 323 - "Community 323"
Cohesion: 0.25
Nodes (8): diagnostics, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 324 - "Community 324"
Cohesion: 0.25
Nodes (8): errors-in-console, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 325 - "Community 325"
Cohesion: 0.25
Nodes (8): final-screenshot, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 326 - "Community 326"
Cohesion: 0.25
Nodes (8): first-contentful-paint, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 327 - "Community 327"
Cohesion: 0.25
Nodes (8): has-hsts, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 328 - "Community 328"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, accesskeys

### Community 329 - "Community 329"
Cohesion: 0.25
Nodes (8): interactive, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 330 - "Community 330"
Cohesion: 0.25
Nodes (8): is-on-https, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 331 - "Community 331"
Cohesion: 0.25
Nodes (8): largest-contentful-paint, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 332 - "Community 332"
Cohesion: 0.25
Nodes (8): main-thread-tasks, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 333 - "Community 333"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-allowed-attr

### Community 334 - "Community 334"
Cohesion: 0.25
Nodes (8): network-rtt, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 335 - "Community 335"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-treeitem-name

### Community 336 - "Community 336"
Cohesion: 0.25
Nodes (8): origin-isolation, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 337 - "Community 337"
Cohesion: 0.25
Nodes (8): redirects-http, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 338 - "Community 338"
Cohesion: 0.25
Nodes (8): resource-summary, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 339 - "Community 339"
Cohesion: 0.25
Nodes (8): screenshot-thumbnails, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 340 - "Community 340"
Cohesion: 0.25
Nodes (8): script-treemap-data, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 341 - "Community 341"
Cohesion: 0.25
Nodes (8): speed-index, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 342 - "Community 342"
Cohesion: 0.25
Nodes (8): third-party-cookies, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 343 - "Community 343"
Cohesion: 0.25
Nodes (8): total-blocking-time, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 344 - "Community 344"
Cohesion: 0.25
Nodes (8): trusted-types-xss, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 345 - "Community 345"
Cohesion: 0.25
Nodes (8): valid-source-maps, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 346 - "Community 346"
Cohesion: 0.33
Nodes (6): empty-heading, description, id, score, scoreDisplayMode, title

### Community 347 - "Community 347"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-conditional-attr

### Community 348 - "Community 348"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-dialog-name

### Community 349 - "Community 349"
Cohesion: 0.33
Nodes (6): label, description, id, score, scoreDisplayMode, title

### Community 350 - "Community 350"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-input-field-name

### Community 351 - "Community 351"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-meter-name

### Community 352 - "Community 352"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-children

### Community 353 - "Community 353"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-required-parent

### Community 354 - "Community 354"
Cohesion: 0.25
Nodes (8): description, errorMessage, errorStack, id, score, scoreDisplayMode, title, aria-text

### Community 355 - "Community 355"
Cohesion: 0.25
Nodes (8): clickjacking-mitigation, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 356 - "Community 356"
Cohesion: 0.25
Nodes (8): csp-xss, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 357 - "Community 357"
Cohesion: 0.25
Nodes (8): cumulative-layout-shift, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 358 - "Community 358"
Cohesion: 0.25
Nodes (8): deprecations, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 359 - "Community 359"
Cohesion: 0.25
Nodes (8): diagnostics, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 360 - "Community 360"
Cohesion: 0.25
Nodes (8): errors-in-console, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 361 - "Community 361"
Cohesion: 0.25
Nodes (8): final-screenshot, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 362 - "Community 362"
Cohesion: 0.25
Nodes (8): first-contentful-paint, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 363 - "Community 363"
Cohesion: 0.25
Nodes (8): has-hsts, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 364 - "Community 364"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-conditional-attr

### Community 365 - "Community 365"
Cohesion: 0.25
Nodes (8): interactive, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 366 - "Community 366"
Cohesion: 0.25
Nodes (8): is-on-https, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 367 - "Community 367"
Cohesion: 0.25
Nodes (8): largest-contentful-paint, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 368 - "Community 368"
Cohesion: 0.25
Nodes (8): main-thread-tasks, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 369 - "Community 369"
Cohesion: 0.33
Nodes (6): link-name, description, id, score, scoreDisplayMode, title

### Community 370 - "Community 370"
Cohesion: 0.25
Nodes (8): network-rtt, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 371 - "Community 371"
Cohesion: 0.25
Nodes (8): origin-isolation, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 372 - "Community 372"
Cohesion: 0.25
Nodes (8): redirects-http, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 373 - "Community 373"
Cohesion: 0.25
Nodes (8): resource-summary, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 374 - "Community 374"
Cohesion: 0.25
Nodes (8): screenshot-thumbnails, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 375 - "Community 375"
Cohesion: 0.25
Nodes (8): script-treemap-data, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 376 - "Community 376"
Cohesion: 0.25
Nodes (8): speed-index, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 377 - "Community 377"
Cohesion: 0.25
Nodes (8): third-party-cookies, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 378 - "Community 378"
Cohesion: 0.25
Nodes (8): total-blocking-time, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 379 - "Community 379"
Cohesion: 0.25
Nodes (8): trusted-types-xss, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 380 - "Community 380"
Cohesion: 0.25
Nodes (8): valid-source-maps, description, errorMessage, errorStack, id, score, scoreDisplayMode, title

### Community 381 - "Community 381"
Cohesion: 0.25
Nodes (7): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 382 - "Community 382"
Cohesion: 0.25
Nodes (7): Arquitectura propuesta (resumen), Idea central, Preguntas abiertas (resolver antes de planificar implementación), Próximo paso, Quick wins independientes (no bloqueados por lo anterior), Roadmap — Cadena de frío / termografía como core del negocio, Roadmap por fases

### Community 383 - "Community 383"
Cohesion: 0.29
Nodes (7): 6. API REST, Endpoints (~50 rutas), Hospitales, Otros, Proyectos, Seguridad de APIs, Visitas

### Community 384 - "Community 384"
Cohesion: 0.33
Nodes (6): 9. Pendiente, Activo (bloqueado por dependencias externas), Activo (pendiente tecnico menor), Backlog — Features nuevas (priorizadas por impacto/esfuerzo), Backlog — Incidencias (modulo aparte), Backlog — Infraestructura (no priorizado)

### Community 385 - "Community 385"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-allowed-attr

### Community 386 - "Community 386"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-command-name

### Community 387 - "Community 387"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-conditional-attr

### Community 388 - "Community 388"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-deprecated-role

### Community 389 - "Community 389"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-dialog-name

### Community 390 - "Community 390"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-hidden-body

### Community 391 - "Community 391"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-hidden-focus

### Community 392 - "Community 392"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-input-field-name

### Community 393 - "Community 393"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-meter-name

### Community 394 - "Community 394"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-progressbar-name

### Community 395 - "Community 395"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-prohibited-attr

### Community 396 - "Community 396"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-roles

### Community 397 - "Community 397"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-toggle-field-name

### Community 398 - "Community 398"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-tooltip-name

### Community 399 - "Community 399"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-valid-attr

### Community 400 - "Community 400"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-valid-attr-value

### Community 401 - "Community 401"
Cohesion: 0.33
Nodes (6): button-name, description, id, score, scoreDisplayMode, title

### Community 402 - "Community 402"
Cohesion: 0.33
Nodes (6): bypass, description, id, score, scoreDisplayMode, title

### Community 403 - "Community 403"
Cohesion: 0.33
Nodes (6): color-contrast, description, id, score, scoreDisplayMode, title

### Community 404 - "Community 404"
Cohesion: 0.33
Nodes (6): custom-controls-labels, description, id, score, scoreDisplayMode, title

### Community 405 - "Community 405"
Cohesion: 0.33
Nodes (6): custom-controls-roles, description, id, score, scoreDisplayMode, title

### Community 406 - "Community 406"
Cohesion: 0.33
Nodes (6): definition-list, description, id, score, scoreDisplayMode, title

### Community 407 - "Community 407"
Cohesion: 0.33
Nodes (6): dlitem, description, id, score, scoreDisplayMode, title

### Community 408 - "Community 408"
Cohesion: 0.33
Nodes (6): duplicate-id-aria, description, id, score, scoreDisplayMode, title

### Community 409 - "Community 409"
Cohesion: 0.33
Nodes (6): empty-heading, description, id, score, scoreDisplayMode, title

### Community 410 - "Community 410"
Cohesion: 0.33
Nodes (6): form-field-multiple-labels, description, id, score, scoreDisplayMode, title

### Community 411 - "Community 411"
Cohesion: 0.33
Nodes (6): heading-order, description, id, score, scoreDisplayMode, title

### Community 412 - "Community 412"
Cohesion: 0.33
Nodes (6): html-has-lang, description, id, score, scoreDisplayMode, title

### Community 413 - "Community 413"
Cohesion: 0.33
Nodes (6): html-lang-valid, description, id, score, scoreDisplayMode, title

### Community 414 - "Community 414"
Cohesion: 0.33
Nodes (6): html-xml-lang-mismatch, description, id, score, scoreDisplayMode, title

### Community 415 - "Community 415"
Cohesion: 0.33
Nodes (6): identical-links-same-purpose, description, id, score, scoreDisplayMode, title

### Community 416 - "Community 416"
Cohesion: 0.33
Nodes (6): image-alt, description, id, score, scoreDisplayMode, title

### Community 417 - "Community 417"
Cohesion: 0.33
Nodes (6): input-image-alt, description, id, score, scoreDisplayMode, title

### Community 418 - "Community 418"
Cohesion: 0.33
Nodes (6): label-content-name-mismatch, description, id, score, scoreDisplayMode, title

### Community 419 - "Community 419"
Cohesion: 0.33
Nodes (6): landmark-one-main, description, id, score, scoreDisplayMode, title

### Community 420 - "Community 420"
Cohesion: 0.33
Nodes (6): link-in-text-block, description, id, score, scoreDisplayMode, title

### Community 421 - "Community 421"
Cohesion: 0.33
Nodes (6): link-name, description, id, score, scoreDisplayMode, title

### Community 422 - "Community 422"
Cohesion: 0.33
Nodes (6): meta-refresh, description, id, score, scoreDisplayMode, title

### Community 423 - "Community 423"
Cohesion: 0.33
Nodes (6): meta-viewport, description, id, score, scoreDisplayMode, title

### Community 424 - "Community 424"
Cohesion: 0.33
Nodes (6): object-alt, description, id, score, scoreDisplayMode, title

### Community 425 - "Community 425"
Cohesion: 0.33
Nodes (6): select-name, description, id, score, scoreDisplayMode, title

### Community 426 - "Community 426"
Cohesion: 0.33
Nodes (6): tabindex, description, id, score, scoreDisplayMode, title

### Community 427 - "Community 427"
Cohesion: 0.33
Nodes (6): table-duplicate-name, description, id, score, scoreDisplayMode, title

### Community 428 - "Community 428"
Cohesion: 0.33
Nodes (6): target-size, description, id, score, scoreDisplayMode, title

### Community 429 - "Community 429"
Cohesion: 0.33
Nodes (6): td-has-header, description, id, score, scoreDisplayMode, title

### Community 430 - "Community 430"
Cohesion: 0.33
Nodes (6): th-has-data-cells, description, id, score, scoreDisplayMode, title

### Community 431 - "Community 431"
Cohesion: 0.33
Nodes (6): valid-lang, description, id, score, scoreDisplayMode, title

### Community 432 - "Community 432"
Cohesion: 0.33
Nodes (6): video-caption, description, id, score, scoreDisplayMode, title

### Community 433 - "Community 433"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-command-name

### Community 434 - "Community 434"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-conditional-attr

### Community 435 - "Community 435"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-deprecated-role

### Community 436 - "Community 436"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-dialog-name

### Community 437 - "Community 437"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-hidden-body

### Community 438 - "Community 438"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-hidden-focus

### Community 439 - "Community 439"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-meter-name

### Community 440 - "Community 440"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-progressbar-name

### Community 441 - "Community 441"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-prohibited-attr

### Community 442 - "Community 442"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-required-attr

### Community 443 - "Community 443"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-required-children

### Community 444 - "Community 444"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-required-parent

### Community 445 - "Community 445"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-toggle-field-name

### Community 446 - "Community 446"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-tooltip-name

### Community 447 - "Community 447"
Cohesion: 0.33
Nodes (6): table-fake-caption, description, id, score, scoreDisplayMode, title

### Community 448 - "Community 448"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-valid-attr

### Community 449 - "Community 449"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-valid-attr-value

### Community 450 - "Community 450"
Cohesion: 0.33
Nodes (6): button-name, description, id, score, scoreDisplayMode, title

### Community 451 - "Community 451"
Cohesion: 0.33
Nodes (6): color-contrast, description, id, score, scoreDisplayMode, title

### Community 452 - "Community 452"
Cohesion: 0.33
Nodes (6): custom-controls-labels, description, id, score, scoreDisplayMode, title

### Community 453 - "Community 453"
Cohesion: 0.33
Nodes (6): custom-controls-roles, description, id, score, scoreDisplayMode, title

### Community 454 - "Community 454"
Cohesion: 0.33
Nodes (6): definition-list, description, id, score, scoreDisplayMode, title

### Community 455 - "Community 455"
Cohesion: 0.33
Nodes (6): document-title, description, id, score, scoreDisplayMode, title

### Community 456 - "Community 456"
Cohesion: 0.33
Nodes (6): duplicate-id-aria, description, id, score, scoreDisplayMode, title

### Community 457 - "Community 457"
Cohesion: 0.33
Nodes (6): valid-lang, description, id, score, scoreDisplayMode, title

### Community 458 - "Community 458"
Cohesion: 0.33
Nodes (6): form-field-multiple-labels, description, id, score, scoreDisplayMode, title

### Community 459 - "Community 459"
Cohesion: 0.11
Nodes (19): image-redundant-alt, input-button-name, link-name, description, id, score, scoreDisplayMode, title (+11 more)

### Community 460 - "Community 460"
Cohesion: 0.33
Nodes (6): html-has-lang, description, id, score, scoreDisplayMode, title

### Community 461 - "Community 461"
Cohesion: 0.33
Nodes (6): html-lang-valid, description, id, score, scoreDisplayMode, title

### Community 462 - "Community 462"
Cohesion: 0.33
Nodes (6): html-xml-lang-mismatch, description, id, score, scoreDisplayMode, title

### Community 463 - "Community 463"
Cohesion: 0.33
Nodes (6): identical-links-same-purpose, description, id, score, scoreDisplayMode, title

### Community 464 - "Community 464"
Cohesion: 0.33
Nodes (6): image-redundant-alt, description, id, score, scoreDisplayMode, title

### Community 465 - "Community 465"
Cohesion: 0.33
Nodes (6): duplicate-id-aria, description, id, score, scoreDisplayMode, title

### Community 466 - "Community 466"
Cohesion: 0.33
Nodes (6): link-in-text-block, description, id, score, scoreDisplayMode, title

### Community 467 - "Community 467"
Cohesion: 0.33
Nodes (6): empty-heading, description, id, score, scoreDisplayMode, title

### Community 468 - "Community 468"
Cohesion: 0.33
Nodes (6): meta-refresh, description, id, score, scoreDisplayMode, title

### Community 469 - "Community 469"
Cohesion: 0.33
Nodes (6): meta-viewport, description, id, score, scoreDisplayMode, title

### Community 470 - "Community 470"
Cohesion: 0.33
Nodes (6): object-alt, description, id, score, scoreDisplayMode, title

### Community 471 - "Community 471"
Cohesion: 0.33
Nodes (6): select-name, description, id, score, scoreDisplayMode, title

### Community 472 - "Community 472"
Cohesion: 0.33
Nodes (6): skip-link, description, id, score, scoreDisplayMode, title

### Community 473 - "Community 473"
Cohesion: 0.33
Nodes (6): table-duplicate-name, description, id, score, scoreDisplayMode, title

### Community 474 - "Community 474"
Cohesion: 0.33
Nodes (6): target-size, description, id, score, scoreDisplayMode, title

### Community 475 - "Community 475"
Cohesion: 0.50
Nodes (4): 13. Estado actual y deuda tecnica, Backlog (no priorizado), Completado (sprints 1-18 + hardening corporativo), Deuda tecnica activa

### Community 476 - "Community 476"
Cohesion: 0.33
Nodes (6): th-has-data-cells, description, id, score, scoreDisplayMode, title

### Community 477 - "Community 477"
Cohesion: 0.33
Nodes (6): definition-list, description, id, score, scoreDisplayMode, title

### Community 478 - "Community 478"
Cohesion: 0.33
Nodes (6): select-name, description, id, score, scoreDisplayMode, title

### Community 479 - "Community 479"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-command-name

### Community 480 - "Community 480"
Cohesion: 0.33
Nodes (6): tabindex, description, id, score, scoreDisplayMode, title

### Community 481 - "Community 481"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-deprecated-role

### Community 482 - "Community 482"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-dialog-name

### Community 483 - "Community 483"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-hidden-body

### Community 484 - "Community 484"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-hidden-focus

### Community 485 - "Community 485"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-meter-name

### Community 486 - "Community 486"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-progressbar-name

### Community 487 - "Community 487"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-prohibited-attr

### Community 488 - "Community 488"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-required-parent

### Community 489 - "Community 489"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-roles

### Community 490 - "Community 490"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-text

### Community 491 - "Community 491"
Cohesion: 0.33
Nodes (6): resource-summary, description, id, score, scoreDisplayMode, title

### Community 492 - "Community 492"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-valid-attr

### Community 493 - "Community 493"
Cohesion: 0.33
Nodes (6): description, id, score, scoreDisplayMode, title, aria-valid-attr-value

### Community 494 - "Community 494"
Cohesion: 0.33
Nodes (6): button-name, description, id, score, scoreDisplayMode, title

### Community 495 - "Community 495"
Cohesion: 0.33
Nodes (6): color-contrast, description, id, score, scoreDisplayMode, title

### Community 496 - "Community 496"
Cohesion: 0.33
Nodes (6): custom-controls-labels, description, id, score, scoreDisplayMode, title

### Community 497 - "Community 497"
Cohesion: 0.33
Nodes (6): custom-controls-roles, description, id, score, scoreDisplayMode, title

### Community 498 - "Community 498"
Cohesion: 0.33
Nodes (6): document-title, description, id, score, scoreDisplayMode, title

### Community 499 - "Community 499"
Cohesion: 0.33
Nodes (6): csp-xss, description, id, score, scoreDisplayMode, title

### Community 500 - "Community 500"
Cohesion: 0.33
Nodes (6): deprecations, description, id, score, scoreDisplayMode, title

### Community 501 - "Community 501"
Cohesion: 0.33
Nodes (6): form-field-multiple-labels, description, id, score, scoreDisplayMode, title

### Community 502 - "Community 502"
Cohesion: 0.33
Nodes (6): frame-title, description, id, score, scoreDisplayMode, title

### Community 503 - "Community 503"
Cohesion: 0.33
Nodes (6): heading-order, description, id, score, scoreDisplayMode, title

### Community 504 - "Community 504"
Cohesion: 0.33
Nodes (6): html-has-lang, description, id, score, scoreDisplayMode, title

### Community 505 - "Community 505"
Cohesion: 0.33
Nodes (6): html-lang-valid, description, id, score, scoreDisplayMode, title

### Community 506 - "Community 506"
Cohesion: 0.33
Nodes (6): html-xml-lang-mismatch, description, id, score, scoreDisplayMode, title

### Community 507 - "Community 507"
Cohesion: 0.33
Nodes (6): identical-links-same-purpose, description, id, score, scoreDisplayMode, title

### Community 508 - "Community 508"
Cohesion: 0.33
Nodes (6): image-alt, description, id, score, scoreDisplayMode, title

### Community 509 - "Community 509"
Cohesion: 0.33
Nodes (6): input-image-alt, description, id, score, scoreDisplayMode, title

### Community 510 - "Community 510"
Cohesion: 0.33
Nodes (6): label, description, id, score, scoreDisplayMode, title

### Community 511 - "Community 511"
Cohesion: 0.33
Nodes (6): label-content-name-mismatch, description, id, score, scoreDisplayMode, title

### Community 512 - "Community 512"
Cohesion: 0.33
Nodes (6): landmark-one-main, description, id, score, scoreDisplayMode, title

### Community 513 - "Community 513"
Cohesion: 0.33
Nodes (6): listitem, description, id, score, scoreDisplayMode, title

### Community 514 - "Community 514"
Cohesion: 0.33
Nodes (6): meta-refresh, description, id, score, scoreDisplayMode, title

### Community 516 - "Community 516"
Cohesion: 0.33
Nodes (6): skip-link, description, id, score, scoreDisplayMode, title

### Community 518 - "Community 518"
Cohesion: 0.33
Nodes (6): table-fake-caption, description, id, score, scoreDisplayMode, title

### Community 519 - "Community 519"
Cohesion: 0.33
Nodes (6): target-size, description, id, score, scoreDisplayMode, title

### Community 520 - "Community 520"
Cohesion: 0.33
Nodes (6): td-has-header, description, id, score, scoreDisplayMode, title

### Community 521 - "Community 521"
Cohesion: 0.33
Nodes (6): td-headers-attr, description, id, score, scoreDisplayMode, title

### Community 522 - "Community 522"
Cohesion: 0.33
Nodes (6): th-has-data-cells, description, id, score, scoreDisplayMode, title

### Community 524 - "Community 524"
Cohesion: 0.33
Nodes (6): video-caption, description, id, score, scoreDisplayMode, title

### Community 525 - "Community 525"
Cohesion: 0.33
Nodes (6): csp-xss, description, id, score, scoreDisplayMode, title

### Community 526 - "Community 526"
Cohesion: 0.33
Nodes (6): diagnostics, description, id, score, scoreDisplayMode, title

### Community 527 - "Community 527"
Cohesion: 0.33
Nodes (6): errors-in-console, description, id, score, scoreDisplayMode, title

### Community 530 - "Community 530"
Cohesion: 0.33
Nodes (6): diagnostics, description, id, score, scoreDisplayMode, title

### Community 533 - "Community 533"
Cohesion: 0.40
Nodes (5): 2. Reglas criticas — romper esto causa bugs en produccion, NextAuth v5, Prisma 7, Roles, URL routing

### Community 534 - "Community 534"
Cohesion: 0.40
Nodes (5): 1. Vision general, Para quien, Que es, URL de produccion, Uso privado

### Community 535 - "Community 535"
Cohesion: 0.40
Nodes (5): 3. Arquitectura, Estructura de directorios, Flujo de una peticion, Grafo de dependencias (Graphify), Patron general

### Community 536 - "Community 536"
Cohesion: 0.40
Nodes (5): 9. Infraestructura, Base de datos, Deploy (Railway), PWA, Redis (Upstash)

### Community 537 - "Community 537"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 538 - "Community 538"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 539 - "Community 539"
Cohesion: 0.50
Nodes (3): For /graphify explain, For /graphify path, graphify reference: query, path, explain

### Community 540 - "Community 540"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 542 - "Community 542"
Cohesion: 0.50
Nodes (4): 11. Testing y calidad, Accesibilidad, Lighthouse (ultima auditoria), Playwright E2E

### Community 543 - "Community 543"
Cohesion: 0.50
Nodes (4): 2. Stack tecnico, Dependencias clave, Dependencias de desarrollo, Tecnologias principales

### Community 544 - "Community 544"
Cohesion: 0.50
Nodes (4): 8. Frontend — Componentes y patrones, Componentes clave, Hooks custom, Patrones de UI

## Knowledge Gaps
- **4033 isolated node(s):** `SessionStart`, `PreToolUse`, `allow`, `bool`, `eslintConfig` (+4028 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **15 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `audits` connect `Community 184` to `Community 10`, `Community 11`, `Community 17`, `Community 276`, `Community 277`, `Community 278`, `Community 279`, `Community 280`, `Community 281`, `Community 282`, `Community 283`, `Community 284`, `Community 285`, `Community 286`, `Community 287`, `Community 288`, `Community 289`, `Community 290`, `Community 291`, `Community 292`, `Community 293`, `Community 295`, `Community 296`, `Community 297`, `Community 298`, `Community 299`, `Community 300`, `Community 301`, `Community 302`, `Community 303`, `Community 304`, `Community 305`, `Community 306`, `Community 179`, `Community 180`, `Community 181`, `Community 182`, `Community 183`, `Community 307`, `Community 308`, `Community 309`, `Community 310`, `Community 205`, `Community 220`, `Community 221`, `Community 222`, `Community 223`, `Community 224`, `Community 225`, `Community 115`, `Community 123`?**
  _High betweenness centrality (0.131) - this node is a cross-community bridge._
- **Why does `redirects` connect `Community 10` to `Community 201`, `Community 204`, `Community 173`, `Community 174`, `Community 175`, `Community 176`, `Community 177`, `Community 178`, `Community 115`, `Community 180`, `Community 205`, `Community 182`, `Community 183`, `Community 179`, `Community 181`, `Community 184`?**
  _High betweenness centrality (0.114) - this node is a cross-community bridge._
- **Why does `audits` connect `Community 187` to `Community 129`, `Community 132`, `Community 134`, `Community 8`, `Community 17`, `Community 29`, `Community 311`, `Community 312`, `Community 313`, `Community 314`, `Community 185`, `Community 316`, `Community 189`, `Community 190`, `Community 186`, `Community 315`, `Community 188`, `Community 317`, `Community 318`, `Community 319`, `Community 320`, `Community 321`, `Community 322`, `Community 323`, `Community 324`, `Community 325`, `Community 326`, `Community 327`, `Community 329`, `Community 330`, `Community 331`, `Community 208`, `Community 209`, `Community 332`, `Community 334`, `Community 336`, `Community 337`, `Community 338`, `Community 339`, `Community 340`, `Community 341`, `Community 342`, `Community 343`, `Community 344`, `Community 345`, `Community 227`, `Community 228`, `Community 229`, `Community 230`, `Community 231`, `Community 117`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Are the 57 inferred relationships involving `checkRateLimit()` (e.g. with `DELETE()` and `GET()`) actually correct?**
  _`checkRateLimit()` has 57 INFERRED edges - model-reasoned connections that need verification._
- **What connects `Hook SessionStart — inyecta CONTEXT.md y estado del grafo graphify. Ejecutado au`, `SessionStart`, `PreToolUse` to the rest of the system?**
  _4064 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Community 0` be split into smaller, more focused modules?**
  _Cohesion score 0.03711843711843712 - nodes in this community are weakly interconnected._
- **Should `Community 1` be split into smaller, more focused modules?**
  _Cohesion score 0.03711843711843712 - nodes in this community are weakly interconnected._