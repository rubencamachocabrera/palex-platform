...
const COORDS_POR_CIUDAD: Record<string, [number, number]> = {
  ...
  "torrejón de ardoz":         [40.4608, -3.4760],
  "parla":                     [40.2376, -3.7757],
  "marbella":                  [36.5101, -4.8858],
  "torrevieja":                [37.9782, -0.6844],
  "benidorm":                  [38.5396, -0.1338],
...
function getCoords(h: HospitalMapa): [number, number] | null {
  ...
  const key = h.ciudad.toLowerCase().trim()
  return COORDS_POR_CIUDAD[key] ?? null
}
...
export default function MapaLeaflet({ hospitales }: { hospitales: HospitalMapa[] }) {
  ...
  const zonasPaleta = useMemo(() => {
    return [...new Map(hospitales.map(h => [h.zona.id, h.zona.nombre])).entries()]
      .sort((a, b) => a[1].localeCompare(b[1]))
  ...
  const sinUbicar = hospitales.length - ubicados.length
  ...
  useEffect(() => {
    ...
    // Give CSS time to load before init
      ...
      {mapReady && zonasPaleta.length > 0 && (
        <div className="absolute bottom-6 left-4 z-[1000] bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100 p-3 max-w-[180px]">
          ...
          <div className="mt-2 pt-2 border-t border-gray-100">
            ...
            {sinUbicar > 0 && <p className="text-[10px] text-amber-500 mt-0.5">{sinUbicar} sin coordenadas</p>}
          </div>
        </div>
      )}
        ...
        <div className="absolute right-0 top-0 h-full w-72 bg-white border-l border-gray-100 shadow-2xl z-[1001] flex flex-col">
          ...
          <div className="flex items-start gap-3 px-5 py-4 border-b border-gray-100">
            ...
            <button
              ...
              className="shrink-0 w-7 h-7 flex items-center justify-center rounded-lg text-gray-300 hover:text-gray-600 hover:bg-gray-100 transition-colors text-base mt-0.5"
              aria-label="Cerrar"
            >
              ×
            </button>
          ...
          <div className="px-5 py-4 grid grid-cols-2 gap-3">
            ...
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              ...
              <p className="text-[10px] text-gray-400 mt-0.5 font-medium uppercase tracking-wide">Ops.</p>
            </div>
          </div>
