...
export async function PATCH(
  ...
  const rl = await checkRateLimit(req as NextRequest, "/api/proyectos/entradas", { limit: 30 })
  if (rl) return rl
  ...
  try {
    ...
    const entrada = await db.entradaTimeline.findFirst({ where: { id: entradaId, proyectoId: id }, select: { id: true } })
...
export async function DELETE(
  ...
  const rl = await checkRateLimit(_req as NextRequest, "/api/proyectos/entradas", { limit: 30 })
  if (rl) return rl
  const session = await auth()
  if (!session?.user) return NextResponse.json({ error: "No autorizado" }, { status: 401 })
  const { id, entradaId } = await params
  try {
    ...
    const result = await db.entradaTimeline.deleteMany({ where: { id: entradaId, proyectoId: id } })
